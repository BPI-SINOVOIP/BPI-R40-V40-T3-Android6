...
int BasicMBRData::ReadMBRData(DiskIO * theDisk, int checkBlockSize) {
   ...
   if (err) {
      ...
      if((code[3] == 0x4d && code[4] == 0x53 && code[5] == 0x44 && code[6]==0x4f && code[7] == 0x53)
		|| (code[3] == 0x45 && code[4] == 0x58 && code[5] == 0x46 && code[6]==0x41 && code[7] == 0x54)
		|| (code[3] == 0x4E && code[4] == 0x54 && code[5] == 0x46 && code[6]==0x53 && code[7] == 0x20))
      {
	      cerr << "found msdos partition!\n";
	      state = msdos; 
      }
   } // no initial error
   return allOK;
