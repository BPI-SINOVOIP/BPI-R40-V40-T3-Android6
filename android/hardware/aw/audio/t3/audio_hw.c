...
static void init_audio_devices(struct sunxi_audio_device *adev)
{
    ...
    //TODO: select device by user define
	property_set("audio.input.active", "AUDIO_CODEC");
	property_set("audio.output.active", "AUDIO_CODEC,AUDIO_I2S,AUDIO_HDMI");
...
static void init_codec_path(struct sunxi_audio_device *adev,int output_device_path ,int input_device_path)
{
    ...
    if (!strcmp(adev->dev_manager[0].card_id, "sndac200")) {
        output_device_id = OUT_DEVICE_HEADPHONES;//OUT_DEVICE_AC200_OUTPUT;
...
static void select_output_device(struct sunxi_audio_device *adev)
{
    ...
    if (adev->stanby) {
        ...
        if (adev->ar == NULL) {
	    ALOGD("select_output_device card %d, init audio route ......", adev->mixer_card);
...
#endif
{
    ...
    for (index = 0; index < MAX_AUDIO_DEVICES; index++) {
        if (adev->dev_manager[index].flag_exist
            ...
            if(!strcmp(adev->dev_manager[index].name,  "AUDIO_CODEC")){
                if(adev->ar == NULL){
                    adev->ar = audio_route_init(adev->mixer_card, AUDIO_XML_PATH);
                    if(adev->ar != NULL)
                        init_codec_path(adev,OUT_DEVICE_HEADPHONES,IN_SOURCE_MAINMIC);                                                                               // end #if
                }
            }
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    ret = str_parms_get_str(parms, AUDIO_PARAMETER_KEY_POWER_STATE, value, sizeof(value));
    if (ret >= 0) {       
        pthread_mutex_lock(&adev->lock);
        if (strcmp(value, AUDIO_PARAMETER_VALUE_ON) == 0) {
            	adev->stanby = true;
                     ...
                     hw_device_t** device)
{
    ...
    for (card = 0; card < MAX_AUDIO_DEVICES; card++) {
        if (adev->dev_manager[card].flag_exist
                && (adev->dev_manager[card].flag_in == AUDIO_IN)
                && !strcmp(adev->dev_manager[card].name, AUDIO_NAME_CODEC)) {
            ALOGV("use card %d %s mixer control", card, adev->dev_manager[card].name);
            break;
        }
    }
    ...
    ALOGV("init T3 codec in adev_open: card %d, %s.", adev->mixer_card, AUDIO_XML_PATH);
    adev->ar = audio_route_init(adev->mixer_card, AUDIO_XML_PATH);
    init_codec_path(adev, OUT_DEVICE_HEADPHONES, IN_SOURCE_MAINMIC);
