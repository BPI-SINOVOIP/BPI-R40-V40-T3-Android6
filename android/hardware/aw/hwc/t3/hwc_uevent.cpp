...
int hwc_tv_switch(int DisplayNum, bool plug, disp_tv_mode set_mode)
{
	{
    SUNXI_hwcdev_context_t *Globctx = &gSunxiHwcDevice;
    int vir_disp = -1;
    ...
    if(plug)
    {
        ...
        {
        	ALOGD("###Allready SET_MODE ::ture(%d)####", set_mode);
            ...
            ALOGD("###has no fix cvbs Mode###");
            return 0;
        }       
        ALOGD( "###CVBS plug in, Type:%d, Mode:0x%08x###",
...
static int hwc_uevent(void)
{
    ...
    struct pollfd fds;
    bool stop_hwc = 0;
    int count, IsVsync, IsHdmi, IsCvbs,hotplug_sock, retval, display_id = -1;
    unsigned int cout = 0, new_hdmi_hpd = 0,new_tv_hpd=0;
	...
	while(1) 
	{
        ...
        if(retval > 0 && (fds.revents & POLLIN))
        {
    		...
    		if(count > 0)
    		{
				...
				IsCvbs= !strcmp(s,"change@/devices/virtual/switch/tve1_cvbs");	
                s += strlen(s) + 1;
                if(IsVsync)
                {
                    timestamp = 0;
                ...
                if(IsHdmi)
                {
                    while(s)
                    {
						...
						if(new_hdmi_hpd==1){
							hwc_tv_switch(1, 0, Globctx->tv_mode);
                            hwc_hotplug_switch(1, 1, DISP_TV_MODE_NUM);}
							else
								{
								 hwc_hotplug_switch(1, 0, DISP_TV_MODE_NUM);
							}
                        }
                        s += strlen(s) + 1;
                        if(s - buf >= count || s - buf >= buffersize)
								  ...
								  while(s)
								  {
									  if (!strncmp(s, "SWITCH_STATE=", strlen("SWITCH_STATE=")))
									  {
										  new_tv_hpd = strtoull(s + strlen("SWITCH_STATE="), NULL, 0);
										   ALOGD( "####disp=%d##hotplug=%d###new_tv_hpd=%d", CVBS_USED, !!new_tv_hpd,new_tv_hpd);
										   if(new_tv_hpd==1){
										   	ALOGD( "####hwc_settv_out_pulgin");
										   Globctx->tv_mode = DISP_TV_MOD_NTSC;
										    hwc_hotplug_switch(1, 0, DISP_TV_MODE_NUM);
						                    hwc_tv_switch(1, 1, Globctx->tv_mode);
										 	...
										 	{
										 	ALOGD( "####hwc_set tvout pulgout");
										 	Globctx->tv_mode = DISP_TV_MODE_NUM;
											hwc_tv_switch(1, 0, Globctx->tv_mode);
										 }
									  }
									  s += strlen(s) + 1;
