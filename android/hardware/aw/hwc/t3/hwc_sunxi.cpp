...
#define CVBS_PERCENT_WIDTH  90//90
#define CVBS_PERCENT_HEIGHT 89//89
#define CVBS_SCREEN_XPOS    (0)   //22
#define CVBS_SCREEN_YPOS    (0)  //12
...
static bool recount_present(DisplayInfo *PsDisplayInfo)
{
    ...
    }else if(PsDisplayInfo->DisplayType == DISP_OUTPUT_TYPE_TV){
        PsDisplayInfo->PersentHeight = CVBS_PERCENT_HEIGHT;
        PsDisplayInfo->PersentWidth = CVBS_PERCENT_WIDTH;
    }
	else{
		PsDisplayInfo->PersentHeight = 100;
        PsDisplayInfo->PersentWidth = 100;
        ...
        disp_layer_info *layer_info, layer_info_t *hwc_layer)
{
    ...
    if(layer_info->b_trd_out == 1)
    {
		...
		//	if (layer_info->screen_win.x >= CVBS_SCREEN_XPOS) {
			//	layer_info->screen_win.x -= CVBS_SCREEN_XPOS;
			//	layer_info->screen_win.y += CVBS_SCREEN_YPOS;
		//	} else {
			//	layer_info->screen_win.x = 0;
			//	layer_info->screen_win.y += CVBS_SCREEN_YPOS;
			//}
...
static int hwc_init_display(void)
{
        ...
        DispCnt++)
    {
        ...
        switch(outtype)
        {
            ...
            case DISP_OUTPUT_TYPE_TV:
				...
				PsDisplayInfo->DisplayMode=DISP_TV_MOD_NTSC;
				ALOGD("############outtype:%d#########virtualtohwdispaly:%d", PsDisplayInfo->DisplayType,PsDisplayInfo->VirtualToHWDisplay );
            case DISP_OUTPUT_TYPE_VGA:
                permanentdisp++;
            case DISP_OUTPUT_TYPE_HDMI:
                PsDisplayInfo->VirtualToHWDisplay = DispCnt;
	...
	else if(permanentdisp && PsDisplayInfo->DisplayType == DISP_OUTPUT_TYPE_TV)
		{
		DispCnt = 0;
        int DispT = PsDisplayInfo->DisplayType, VtoH = PsDisplayInfo->VirtualToHWDisplay;
        while(DispCnt < hasDispCnt && Globctx->SunxiDisplay[0].DisplayType == DISP_OUTPUT_TYPE_TV)
        {
            DispCnt++;
            if(PsDisplayInfo[DispCnt].DisplayType != DISP_OUTPUT_TYPE_TV
                && PsDisplayInfo[DispCnt].DisplayType != DISP_OUTPUT_TYPE_NONE)
            {
                PsDisplayInfo->VirtualToHWDisplay = PsDisplayInfo[DispCnt].VirtualToHWDisplay;
                PsDisplayInfo->DisplayType = PsDisplayInfo[DispCnt].DisplayType;
            ...
            switch(PsDisplayInfo->DisplayType)
            {
                ...
                case DISP_OUTPUT_TYPE_VGA:	
                    ...
                    if(PsDisplayInfo == Globctx->SunxiDisplay)
                    {
						...
						hwc_tv_switch(PsDisplayInfo->VirtualToHWDisplay, 1, DISP_TV_MODE_NUM);
                    }
                    PsDisplayInfo->issecure = 1;
                    PsDisplayInfo->active = 1;
                    break;
...
SUNXI_hwcdev_context_t* hwc_create_device(void)
{
	...
	open_fd = open("/sys/class/switch/tve1_cvbs/state", O_RDONLY);
    if (open_fd >= 0)
    {
        char val;
        if (read(open_fd, &val, 1) == 1 && val == '1')
        {
            if(Globctx->SunxiDisplay[0].DisplayType != DISP_OUTPUT_TYPE_TV
                && Globctx->SunxiDisplay[1].VirtualToHWDisplay == -EINVAL)
            {
            	Globctx->tv_mode = DISP_TV_MOD_NTSC;
                hwc_tv_switch(1, 1, Globctx->tv_mode);
            }
            ALOGD("### init cvbs : IN ###");
        }else{
            ALOGD("### init cvbs: OUT ###");
        ...
        ALOGD("###open /sys/class/switch/cvbs/state fail");
    }
