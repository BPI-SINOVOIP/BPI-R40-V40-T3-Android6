...
public class EthernetConfigStore extends IpConfigStore {
    ...
    private static final String staticIpConfigFile = Environment.getDataDirectory() +
            "/misc/ethernet/static_ipconfig.txt";
    private static final String pppoeConfigFile = Environment.getDataDirectory() +
            "/misc/ethernet/pppoe_ipconfig.txt";
    ...
    public IpConfiguration readIpAndProxyConfigurations() {
        ...
        if (networks.size() == 0) {
            ...
            return new IpConfiguration(IpAssignment.DHCP, ProxySettings.NONE,null, null, null);
        }
    ...
    public IpConfiguration readIpAndProxyConfigurations(IpAssignment type) {
        SparseArray<IpConfiguration> networks = new SparseArray<IpConfiguration>();
        if (IpAssignment.STATIC == type) {
            Log.d(TAG, "get saved ethernet static ip config");
            networks = readIpAndProxyConfigurations(staticIpConfigFile);
        } else if (IpAssignment.PPPOE == type) {
            Log.d(TAG, "get saved ethernet pppoe config");
            networks = readIpAndProxyConfigurations(pppoeConfigFile);
        } else {
            Log.w(TAG, "not static or pppoe, you get config same as readIpAndProxyConfigurations()");
