...
public class Camera {
	...
	//waterMark start
	private boolean mWaterMarkRunning = false;
	//waterMark end
	...
	public native final void stopRender();
    public native final void startRender();
    /**
	...
	*/
	...
	public static int getCVBSInStatus(int cameraId)
	{
		int status = CVBS_IN_STATUS_OUT;
		IBinder b = ServiceManager.getService(Context.CVBS_IN_SERVICE);
        ICvbsInManager cvbsService = ICvbsInManager.Stub.asInterface(b);
		
        try {
            status = cvbsService.getStatus(cameraId);
        } catch (RemoteException e) {
	...
	*@saturation  0-255
	*/
	public native final void setAnalogInputColor(int brightness,int contrast,int saturation);
    ...
    public static final int CAMERA_ERROR_SERVER_DIED = 100;
	 ...
	 /*
         * Allwinnertech SetWaterMark
         * startWaterMarke: display string that set by setWaterMarkMultiple
         * on the camera image
         * use stopWaterMark to cancel watermark display
         * 20160114
         */
    ...
    private native final void _startWaterMark();
    private native final void _stopWaterMark();
    public final void startWaterMark() {
        if (mWaterMarkRunning) {
            throw new RuntimeException("water mark is already running");
        }
        _startWaterMark();
        mWaterMarkRunning = true;
    }

    public final void stopWaterMark() {
        _stopWaterMark();
        mWaterMarkRunning = false;
    }

    private native final void _setWaterMarkMultiple(String mWaterMark,int dispMode);
    public void setWaterMarkMultiple(String mWaterMark){
        //Log.v(TAG,mWaterMark);
        _setWaterMarkMultiple(mWaterMark,WaterMarkDispMode.VIDEO_AND_PREVIEW);
    }
    public void setWaterMarkMultiple(String mWaterMark,int dispMode){
        //Log.v(TAG,mWaterMark);
        if ((dispMode >= WaterMarkDispMode.VIDEO_AND_PREVIEW)
            && (dispMode <= WaterMarkDispMode.VIDEO_ONLY)) {
            _setWaterMarkMultiple(mWaterMark,dispMode);
        } else {
            _setWaterMarkMultiple(mWaterMark,WaterMarkDispMode.VIDEO_AND_PREVIEW);
        }
    }

    /* waterMark 20160114 end */
