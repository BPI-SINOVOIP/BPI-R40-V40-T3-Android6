...
public class VolumeInfo implements Parcelable {
    ...
    public File getPathForUser(int userId) {
	 ...
	 public File getPathForUser(int userId,boolean NickName) {
        if (path == null) {
            return null;
        } else if (type == TYPE_PUBLIC) {
            return new File(path);
        } else if (type == TYPE_EMULATED) {
            return new File(path, "sdcard");
        } else {
            return null;
        }
    }
    ...
    public File getInternalPathForUser(int userId) {
        if (type == TYPE_PUBLIC) {
            ...
            return getPathForUser(userId,true);
        }
    }
    ...
    public StorageVolume buildStorageVolume(Context context, int userId, boolean NickName, boolean reportUnmounted) {
        final StorageManager storage = context.getSystemService(StorageManager.class);
		...
		if(NickName)		
        	userPath = getPathForUser(userId,NickName);
		else
			userPath = getPathForUser(userId);
