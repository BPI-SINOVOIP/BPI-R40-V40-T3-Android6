    ...
    jint cameraId, jobject info_obj)
{
	...
	env->SetIntField(info_obj, fields.is_uvc, cameraInfo.is_uvc);
...
static void android_hardware_Camera_startRender(JNIEnv *env, jobject thiz)
{
    ALOGV("startRender");
    sp<Camera> camera = get_native_camera(env, thiz, NULL);
    if (camera == 0) return;

    status_t rc = camera->sendCommand(CAMERA_CMD_START_RENDER, 0, 0);
    if (rc != NO_ERROR) {
        jniThrowRuntimeException(env, "start Render error");
    }
}

static void android_hardware_Camera_stopRender(JNIEnv *env, jobject thiz)
{
    ALOGV("stopRender");
    sp<Camera> camera = get_native_camera(env, thiz, NULL);
    if (camera == 0) return;

    status_t rc = camera->sendCommand(CAMERA_CMD_STOP_RENDER, 0, 0);
    if (rc != NO_ERROR) {
        jniThrowRuntimeException(env, "stop Render error");
    }
}
...
#ifdef WATERMARK
static void android_hardware_Camera_stopWaterMark(JNIEnv *env, jobject thiz)
{
    ALOGV("stopWaterMark");
    sp<Camera> camera = get_native_camera(env, thiz, NULL);
    if (camera == 0) return;

    if (camera->sendCommand(CAMERA_CMD_STOP_WATER_MARK, 0, 0) != NO_ERROR) {
        jniThrowRuntimeException(env, "stop water mark failed");
    }
}

static void android_hardware_Camera_setWaterMarkMultiple(JNIEnv *env, jobject thiz, jstring mWaterMark, jint dispMode)
{
    ALOGV("setWaterMarkMultiple");
    sp<Camera> camera = get_native_camera(env, thiz, NULL);
    if (camera == 0) return;
    const jchar* strWaterMark = env->GetStringCritical(mWaterMark, 0);

    String8 mWaterMark8;
    if (mWaterMark) {
        mWaterMark8 = String8(reinterpret_cast<const char16_t*>(strWaterMark), env->GetStringLength(mWaterMark));
        env->ReleaseStringCritical(mWaterMark, strWaterMark);
    }
    status_t ret = camera->setWaterMarkMultiple(mWaterMark8, dispMode);
    ALOGV("setWaterMarkMultiple ret = %d",ret);
    if ( ret != NO_ERROR) {
        jniThrowRuntimeException(env, "setWaterMarkMultiple failed");
        return;
    }
}

static void android_hardware_Camera_startWaterMark(JNIEnv *env, jobject thiz)
{
    ALOGV("startWaterMark");
    JNICameraContext* context;
    sp<Camera> camera = get_native_camera(env, thiz, &context);
    if (camera == 0) return;
    status_t rc = camera->sendCommand(CAMERA_CMD_START_WATER_MARK, 0, 0);
    if (rc != NO_ERROR) {
        jniThrowRuntimeException(env, "start water mark failed");
    }
...
static void android_hardware_Camera_setAnalogInputColor(JNIEnv *env, jobject thiz,
	jint brightness,jint contrast,jint saturation)
{
	sp<Camera> camera = get_native_camera(env, thiz, NULL);
    if (camera == 0) return;

    camera->setAnalogInputColor(brightness,contrast,saturation);
}
...
static JNINativeMethod camMethods[] = {
  ...
  { "startRender",
    "()V",
    (void *)android_hardware_Camera_startRender },
  { "stopRender",
    "()V",
    (void *)android_hardware_Camera_stopRender },
#endif
#ifdef WATERMARK
	{ "_startWaterMark",
	  "()V",
	  (void *)android_hardware_Camera_startWaterMark},
	{ "_stopWaterMark",
	  "()V",
	  (void *)android_hardware_Camera_stopWaterMark},
	{ "_setWaterMarkMultiple",
	  "(Ljava/lang/String;I)V",
	  (void *)android_hardware_Camera_setWaterMarkMultiple},
#endif
  ...
  { "setAnalogInputColor",
    "(III)V",
    (void *)android_hardware_Camera_setAnalogInputColor},
...
int register_android_hardware_Camera(JNIEnv *env)
{
    field fields_to_find[] = {
        ...
        { "android/hardware/Camera$CameraInfo", "is_uvc",   "I", &fields.is_uvc },
