...
/*
int start_pppoe(const char * const ifname,
                    const char * const username,
                    const char *ppp_interface,
                    const char *ipaddr,
                    const char *gateway,
                    uint32_t *prefixLength,
                    const char *dns[],
                    ...
                    const char *domains,
                    const char *mtu);
int stop_pppoe(const char *ifname);
...
static jboolean android_net_utils_startPppoe(JNIEnv* env, jobject clazz, jstring ifname,
        jstring username,jobject dhcpResults)
{
    int result=-1;
    /*
    char  ppp_interface[PROPERTY_VALUE_MAX];
    char  ipaddr[PROPERTY_VALUE_MAX];
    uint32_t prefixLength;
    char gateway[PROPERTY_VALUE_MAX];
    char    dns1[PROPERTY_VALUE_MAX];
    ...
    const char *ifnameStr = env->GetStringUTFChars(ifname, NULL);
    if (ifnameStr == NULL) return (jboolean)false;

    const char *usernameStr = env->GetStringUTFChars(username, NULL);
    if (usernameStr == NULL) return (jboolean)false;
    result = ::start_pppoe(ifnameStr, usernameStr, ppp_interface, ipaddr, gateway, &prefixLength,
            dns, server, &lease, vendorInfo, domains, mtu);
    if (result != 0) {
        ALOGD("start_pppoe failed : %s", ifnameStr);
    }
...
static JNINativeMethod gNetworkUtilMethods[] = {
    ...
    { "startPppoe", "(Ljava/lang/String;Ljava/lang/String;Landroid/net/DhcpResults;)Z",  (void *)android_net_utils_startPppoe },
