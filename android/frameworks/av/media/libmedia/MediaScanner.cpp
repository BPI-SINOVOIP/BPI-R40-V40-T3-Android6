...
bool MediaScanner::shouldStopScaner(){
	bool retval = false;
	char state[PROPERTY_VALUE_MAX] = {0};
	property_get("mediasw.stopscaner",state,"0");
	//ALOGD("shouldStopScaner state = %s",state);
	if(!strcmp(state,"1")){
		retval = true;
	}
	return retval;
...
MediaScanResult MediaScanner::doProcessDirectoryEntry(
	...
	*/
	if(strlen(name) >= 12){
		if(!strncmp(name,"$RECYCLE.BIN",12) ){
			ALOGD("ignore rubish file ");
			return MEDIA_SCAN_RESULT_SKIPPED;
		}
	}
	/*end*/
	...
	/*add by liuanlong 14/12/4
	  ignore "BDMV"*/
	if(!strncmp(name,"BDMV",4)){
		char tempPath[2048];
		strcpy(tempPath,path);
		strcat(tempPath,"/STREAM");
		if(access(tempPath,F_OK) == 0){
			ALOGE("---------ignore BDMV/STREAM-----------");
			return MEDIA_SCAN_RESULT_SKIPPED;
		}
	}
	/*end*/
    ...
    if (type == DT_DIR) {
		...
		while(shouldStopScaner()){
			ALOGD("when Media Player is running we skip scan it");
			sleep(2);
		}
    }
