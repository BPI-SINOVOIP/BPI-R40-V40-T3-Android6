...
status_t MediaPlayer::setSpeed_l(int mspeed)
{
	ALOGD("setSpeed %d", mspeed);
	if ((mPlayer != 0) && ( mCurrentState & ( MEDIA_PLAYER_STARTED) ) ) {
		return mPlayer->setSpeed(mspeed);
		}
	ALOGE("Attempt to perform setSpeed in wrong state: mPlayer=%p, mCurrentState=%u", mPlayer.get(), mCurrentState);
	return INVALID_OPERATION;
}

status_t MediaPlayer::setSpeed(int mspeed)
{
	ALOGD("setSpeed %d", mspeed);
	mLockThreadId = getThreadId();
	Mutex::Autolock _l(mLock);
	status_t result = setSpeed_l(mspeed);
	mLockThreadId = 0;
	return result;
}
