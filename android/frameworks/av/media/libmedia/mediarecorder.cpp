...
status_t MediaRecorder::setNextFile(int fd)
{
	ALOGV("setNextFile(%d)", fd);
    if (mMediaRecorder == NULL) {
        ALOGE("media recorder is not initialized yet");
        return INVALID_OPERATION;
    }
    
    status_t ret = mMediaRecorder->setNextFile(fd);
    if (OK != ret) {
        ALOGV("setNextFile failed: %d", ret);
        mCurrentState = MEDIA_RECORDER_ERROR;
        return ret;
    }
    return ret;
...
MediaRecorder::MediaRecorder(const String16& opPackageName,int type) : mSurfaceMediaSource(NULL)
{
    ALOGV("constructor");
    ...
    if (service != NULL) {
		if(type > 0)
		{
			mMediaRecorder = service->createAWMediaRecorder(opPackageName);
		}
		else
		{
