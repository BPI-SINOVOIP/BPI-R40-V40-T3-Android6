...
sp<IMediaRecorder> MediaPlayerService::createAWMediaRecorder(const String16 &opPackageName)
{
    pid_t pid = IPCThreadState::self()->getCallingPid();
    sp<MediaRecorderClient> recorder = new MediaRecorderClient(this, pid, opPackageName,1);
    wp<MediaRecorderClient> w = recorder;
    Mutex::Autolock lock(mLock);
    mMediaRecorderClients.add(w);
    ALOGV("Create new media recorder client from pid %d", pid);
...
status_t MediaPlayerService::Client::setSpeed(int mspeed)
{
	ALOGD("[%d] setSpeed(%d)", mConnId, mspeed);
	sp<MediaPlayerBase> p = getPlayer();
	if (p == 0) return UNKNOWN_ERROR;
	return p->setSpeed(mspeed);
}
