...
MediaRecorderClient::MediaRecorderClient(const sp<MediaPlayerService>& service, pid_t pid,
        const String16& opPackageName,int type)
{
    ALOGV("Client constructor");
    mPid = pid;
	if(type > 0)
	{
		mRecorder = new AWStagefrightRecorder(opPackageName);
	}
	else
	{
...
status_t MediaRecorderClient::setNextFile(int fd)
{
    ...
    return mRecorder->setNextFile(fd);
}
