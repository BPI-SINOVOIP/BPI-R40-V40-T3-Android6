...
int ConfigOutputPicturePixelFormat(VideoDecoder* pDecoder, int ePixelFormat)
{
       ...
       ePixelFormat != PIXEL_FORMAT_YUV_MB32_420)
    {
        if(ePixelFormat<PIXEL_FORMAT_MIN || ePixelFormat>PIXEL_FORMAT_MAX)
            ...
            logw("can not support transform picture to output %s pixel format.", \
                strPixelFormat[ePixelFormat]);
...
{
    CEDARC_UNUSE(pDecoder);
    CEDARC_UNUSE(nMemoryThresh);
    //VideoDecoderContext* p;


    // TODO: thresh feature has bug, not support.
...
static void vdecoderUseFunction()
{
    ...
    CEDARC_UNUSE(ConfigHorizonScaleDownRatio);
    CEDARC_UNUSE(ConfigVerticalScaleDownRatio);
    CEDARC_UNUSE(ConfigRotation);
    CEDARC_UNUSE(ConfigDeinterlace);
    CEDARC_UNUSE(ConfigThumbnailMode);
    CEDARC_UNUSE(ConfigOutputPicturePixelFormat);
    CEDARC_UNUSE(ConfigNoBFrames);
    CEDARC_UNUSE(ConfigDisable3D);
    CEDARC_UNUSE(ConfigVeMemoryThresh);
    ...
    CEDARC_UNUSE(ConfigSecHorizonScaleDownRatio);
    CEDARC_UNUSE(ConfigSecVerticalScaleDownRatio);
