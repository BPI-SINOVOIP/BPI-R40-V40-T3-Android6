...
ssize_t MediaMuxer::addTrack(const sp<MetaData> &trackMeta) {
    Mutex::Autolock autoLock(mMuxerLock);

    if (trackMeta.get() == NULL) {
        ALOGE("addTrack() get a null format");
        return -EINVAL;
    }
...
status_t MediaMuxer::writeSampleData(MediaBuffer* buffer, size_t trackIndex,
                                     int64_t timeUs, uint32_t flags) {
    Mutex::Autolock autoLock(mMuxerLock);
	status_t sta;
    ...
    MediaBuffer* mediaBuffer = new MediaBuffer((void *)((unsigned char *)(buffer->data())+buffer->range_offset()),buffer->range_length());

    mediaBuffer->add_ref(); // Released in MediaAdapter::signalBufferReturned().
    //mediaBuffer->set_range(0, buffer->size());
	
    sp<MetaData> sampleMetaData = mediaBuffer->meta_data();
	buffer->meta_data()->findInt32(kKeyIsCodecConfig, &isCodecConfig);
	sampleMetaData->setInt32(kKeyIsCodecConfig,isCodecConfig);
    sampleMetaData->setInt64(kKeyTime, timeUs);
	...
	sta = currentTrack->pushBuffer(mediaBuffer);
