...
status_t ACodec::setupAVCEncoderParameters(const sp<AMessage> &msg) {
    ...
    int32_t qp_min = 0;
	int32_t qp_max = 0;
    int32_t bit_rate = 6*1024*1024;
	msg->findInt32("qp-min", &qp_min);
	msg->findInt32("qp-max", &qp_max);
    msg->findInt32("bitrate", &bit_rate);

	if (qp_min > 0 && qp_max > 0)
	{
	    OMX_INDEXTYPE index;
		ALOGD("set vbr qp_min=%d,qp_max=%d",qp_min,qp_max);
        err = mOMX->getExtensionIndex(mNode, "OMX.aw.index.param.videoVBRConfig", &index);
        if (err == OK)
        {
            OMX_VIDEO_PARAMS_VBR params;
            params.bEnable = OMX_TRUE;
            params.uQpMin= qp_min;
            params.uQpMax= qp_max;
            params.uBitRate = bit_rate;
            err = mOMX->setParameter(mNode, index, &params, sizeof(params));
        }

        if (err != OK)
        {
            ALOGE("mOMX->setParameter(VBR) failed. (err %d)\n", err);
        }
            return err;
	}
    /***************************** Change for VBR **************************************************/
