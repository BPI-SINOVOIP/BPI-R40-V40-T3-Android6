...
typedef struct DemuxCompContext_t
{
    ...
    // nStartPlaySize is the cache size when start play),
    // nStartPlayCacheSize is the cache size while playing need to buffer)
    int nStartPlaySize;                  //256*1024;
    int nStartPlayCacheSize;             //512*1024;
    int nStartPlayCacheSizeNoVideo;      //10*1024; // just have audio stream
    int nCacheBufferSize;                //20*1024*1024;
    int nCacheBufferSizeLive;            //20*1024*1024;
    int nStartPlayCacheTime;             //5000;   //* 5 seconds.
    int nMaxStartPlayChacheSize;         //150*1024*1024;
    int nMaxCacheBufferSize;             //200*1024*1024;
...
static int setDataSourceFields(DemuxCompContext *demuxHdr,
                               ...
                               void* pHTTPServer,
                               char* uri,
                               KeyedVector<String8,String8>* pHeader);
#else
static int setDataSourceFields(DemuxCompContext *demuxHdr,
                               CdxDataSourceT* source,
                               char* uri,
...
static int base64Encode(const unsigned char *in_ptr, unsigned long encode_len,
...
DemuxComp* DemuxCompCreate(void)
{
    ...
    d->nStartPlaySize = GetConfigParamterInt("start_play_size",256)*1024;
    d->nStartPlayCacheSize = GetConfigParamterInt("start_play_cache_size",512)*1024;
    d->nStartPlayCacheSizeNoVideo = GetConfigParamterInt("start_play_cache_size_no_video",10)*1024;
    ...
    d->nStartPlayCacheTime = GetConfigParamterInt("start_play_cache_time",5)*1000;
    ...
    logv("nStartPlaySize(%d)",d->nStartPlaySize);
    logv("nStartPlayCacheSize(%d)",d->nStartPlayCacheSize);
    logv("nStartPlayCacheSizeNoVideo(%d)",d->nStartPlayCacheSizeNoVideo);
    logv("nCacheBufferSize(%d)",d->nCacheBufferSize);
    logv("nStartPlayCacheTime(%d)",d->nStartPlayCacheTime);
    logv("nMaxStartPlayChacheSize(%d)",d->nMaxStartPlayChacheSize);
    logv("nMaxCacheBufferSize(%d)",d->nMaxCacheBufferSize);
...
void DemuxCompDestroy(DemuxComp* d)
{
    ...
    /* lock mutex to sync with the message process thread when processing SEEK
     * message.  when processing the SEEK message, we clear the Parser's force
     * stop flag by calling CdxParserClrForceStop().  if we're playing a
     * network stream and processing seek message, when user wants to stop, we
     * should set the parser's force stop flag by calling CdxParserForceStop()
     * to prevend from blocking at the network io.
     */
...
#if (CONFIG_OS_VERSION >= OPTION_OS_VERSION_ANDROID_5_0)
int DemuxCompSetUrlSource(DemuxComp* d, void* pHTTPServer, const char* pUrl,
                        const KeyedVector<String8, String8>* pHeaders)
#else
int DemuxCompSetUrlSource(DemuxComp* d, const char* pUrl,
                        const KeyedVector<String8, String8>* pHeaders)
#endif

{
    ...
    setMessage(&msg,
               //* message id.
               DEMUX_COMMAND_SET_SOURCE,
               //* params[0] = &semSetDataSource.
               (uintptr_t)&demux->semSetDataSource,
               //* params[1] = &nSetDataSourceReply.
               (uintptr_t)&demux->nSetDataSourceReply,
               ...
               (uintptr_t)pHeaders,
               //* params[5] = pHTTPServer;
               (uintptr_t)pHTTPServer);
...
int DemuxCompSetFdSource(DemuxComp* d, int fd, int64_t nOffset, int64_t nLength)
{
    ...
    setMessage(&msg,
               ...
               (uintptr_t)(nOffset>>32),
               //* params[5] = low 32 bits of offset.
               (uintptr_t)(nOffset & 0xffffffff),
               //* params[6] = high 32 bits of length.
               (uintptr_t)(nLength>>32),
               //* params[7] = low 32 bits of length.
               (uintptr_t)(nLength & 0xffffffff));
    AwMessageQueuePostMessage(demux->mq, &msg);
    SemTimedWait(&demux->semSetDataSource, -1);
...
int DemuxCompSetStreamSource(DemuxComp* d, const char* pUri)
{
    ...
    setMessage(&msg,
               ...
               //* params[1] = &nSetDataSourceReply.
               (uintptr_t)&demux->nSetDataSourceReply,
               //* params[2] = SOURCE_TYPE_ISTREAMSOURCE.
               SOURCE_TYPE_ISTREAMSOURCE,
               //* params[3] = uri of IStreamSource.
               (uintptr_t)pUri);
    AwMessageQueuePostMessage(demux->mq, &msg);
    SemTimedWait(&demux->semSetDataSource, -1);
...
MediaInfo* DemuxCompGetMediaInfo(DemuxComp* d)
{
    ...
    if (strncmp(demux->source.uri, "dtmb://", 7) == 0)
    {
        mi->is_dtv = 1;
        logd("DTMB stream...");
    }
    ...
    {
        size_t size = sizeof(VideoStreamInfo)*myMediaInfo->nVideoStreamNum;
        pVideoStreamInfo = (VideoStreamInfo*)malloc(size);
        ...
        memset(pVideoStreamInfo, 0, size);
    ...
    {
        size_t size = sizeof(AudioStreamInfo)*myMediaInfo->nAudioStreamNum;
        pAudioStreamInfo = (AudioStreamInfo*)malloc(size);
        ...
        memset(pAudioStreamInfo, 0, size);
    ...
    {
        size_t size = sizeof(SubtitleStreamInfo)*myMediaInfo->nSubtitleStreamNum;
        pSubtitleStreamInfo = (SubtitleStreamInfo*)malloc(size);
        ...
        memset(pSubtitleStreamInfo, 0, size);
...
int DemuxCompCancelSeek(DemuxComp* d)  //* should not call back DEMUX_SEEK_FINISH message.
{
     ...
     * message.  when processing the SEEK message, we clear the Parser's force
     * stop flag by calling CdxParserClrForceStop().  if we're playing a
     * network stream and processing seek message, when user cancel the seek
     * message, we should set the parser's force stop flag by calling
     * CdxParserForceStop() to prevend from blocking at the network io.
     */
    pthread_mutex_lock(&demux->mutex);
    demux->bCancelSeek = 1;
    pthread_mutex_unlock(&demux->mutex);
    ...
    setMessage(&msg, DEMUX_COMMAND_CANCEL_SEEK, (uintptr_t)&demux->semCancelSeek);
    AwMessageQueuePostMessage(demux->mq, &msg);
    ...
    switch(eCachePolicy)
    {
        case CACHE_POLICY_QUICK:
        {
            ...
            if (demux->bLiveStream)
                StreamCacheSetSize(demux->pCache, nStartPlaySize, demux->nCacheBufferSizeLive);
            else
                StreamCacheSetSize(demux->pCache, nStartPlaySize, demux->nCacheBufferSize);
            break;
        }
...
static int DemuxCompSetCacheTime(DemuxCompContext* demux, int nCacheTimeMs)
{
    ...
    if(nStartPlaySize < demux->nStartPlayCacheSize)  //* 128KB.
        nStartPlaySize = demux->nStartPlayCacheSize;
    if(nStartPlaySize > demux->nMaxStartPlayChacheSize)      //* 150MB.
    ...
    if(nMaxBufferSize > demux->nMaxCacheBufferSize)          //* 200MB.
    ...
    {
        nStartPlaySize = demux->nStartPlayCacheSizeNoVideo;
    }
            ...
            */
            ...
            if(demux->nSourceType == SOURCE_TYPE_URL)
            {
                if (!strncasecmp(demux->source.uri, "file://", 7) ||
                        !strncasecmp(demux->source.uri, "bdmv://", 7) ||
                        demux->source.uri[0] == '/')
                    demux->bFileStream = 1;
            ...
            /* if the parser is wvm , we should not start cache thread, because
             * wvm need secure buffer.  if the parser is playready sstr, we
             * need secure buffer.
             */
            ...
            if(demux->bFileStream == 0)
            {
                ...
                //for start playing
                //nStartPlaySize 表示最开始播放要缓冲的数据，
                //而nStartPlayCacheSize表示播放过程缓冲时开始播放的数据大小
                if(demux->mediaInfo.nVideoStreamNum == 0)
                {
                    DemuxCompSetCachePolicy((DemuxComp*)demux, CACHE_POLICY_QUICK,
                            demux->nStartPlayCacheSizeNoVideo, 0, demux->nCacheBufferSize);
                }
                else
                {
                    DemuxCompSetCachePolicy((DemuxComp*)demux, CACHE_POLICY_QUICK,
                            demux->nStartPlaySize, 0, demux->nCacheBufferSize);
                }
            }
            ...
            if(demux->cacheThreadId != 0) //* cache some data before start player.
            {
                while(1)
                {
                    if(StreamCacheDataEnough(demux->pCache) || demux->bEOS || demux->bIOError)
                    {
                        ...
                        if(demux->bEOS)
                        {
                            /* android.media.cts wait for Buffering Update
                             * Notify before MediaPlayer.start
                             */
...
_endTask:
        ...
        else if(msg.messageId == DEMUX_COMMAND_SEEK)
        {
            ...
            if(demux->pParser != NULL)
            {
                 ...
                 * operation or DEmuxCompStop() operation.  when user are
                 * requesting to quit the seek operation, we should not clear
                 * the parser's force stop flag, other wise the parser may
                 * blocked at a network io operation.
                 */
                pthread_mutex_lock(&demux->mutex);
        ...
        else if(msg.messageId == DEMUX_COMMAND_READ)
        {
                ...
                if(demux->bBufferring)
                {
                    ...
                    else
                    {
                        if(demux->bBufferStartNotified == 0)
                        {
                            ...
                            {
                                /* had been paused for buffering for more than
                                 * 2 seconds, notify buffer start message to
                    ...
                    else
                    {
                        ...
                        if(ePlayerMediaType == MEDIA_TYPE_VIDEO && node->infoVersion == 1)
                        {
                            ...
                            if(node->info)
                            {
                                ...
                                VideoStreamInfo *pVideoStreamInfo = (VideoStreamInfo*)malloc(size);
                                if(pVideoStreamInfo == NULL)
                                {
                        ...
                        else if(ePlayerMediaType == MEDIA_TYPE_AUDIO && node->infoVersion == 1)
                        {
                            ...
                            if(node->info)
                            {
                                ...
                                AudioStreamInfo *pAudioStreamInfo = (AudioStreamInfo*)malloc(size);
                                if(pAudioStreamInfo == NULL)
                                {
                                    loge("malloc video stream info fail!");
                                    return NULL;
                                }
                                memset(pAudioStreamInfo, 0, sizeof(size));
                                for(int i = 0; i < pAudioinfo->audioNum; i++)
                                {
                                    ...
                                    pAudioStreamInfo[i].pCodecSpecificData = (char*)malloc(size);
            ...
            else
            {
                ...
                if(packet.type == CDX_MEDIA_VIDEO)
                {
                    ...
                    if ((1 << packet.streamIndex) & demux->mediaInfo.nVideoStreamMask)
                        nStreamIndex = -1;
                }
                else if(packet.type == CDX_MEDIA_AUDIO)
                {
                    ...
                    if ((1 << nStreamIndex) & demux->mediaInfo.nAudioStreamMask)
                        nStreamIndex = -1;
                }
                ...
                {
                    if((!AWPLAYER_CONFIG_DISABLE_VIDEO &&
                                ePlayerMediaType == MEDIA_TYPE_VIDEO && nStreamIndex >= 0) ||
                       (!AWPLAYER_CONFIG_DISABLE_AUDIO &&
                                ePlayerMediaType == MEDIA_TYPE_AUDIO &&
                                nStreamIndex >= 0) ||
                ...
                if(ret == 0)
                {
                    if((!AWPLAYER_CONFIG_DISABLE_VIDEO &&
                                ePlayerMediaType == MEDIA_TYPE_VIDEO && nStreamIndex >= 0) ||
                       (!AWPLAYER_CONFIG_DISABLE_AUDIO &&
                                ePlayerMediaType == MEDIA_TYPE_AUDIO && nStreamIndex >= 0) ||
                       (!AWPLAYER_CONFIG_DISABLE_SUBTITLE &&
                                ePlayerMediaType == MEDIA_TYPE_SUBTITLE))
                    {
...
prefetch_again:
                ...
                if(ret == 0)
                {
                    ...
                    if(packet.type == CDX_MEDIA_VIDEO)
                    {
                        if ((1 << node.nStreamIndex) & demux->mediaInfo.nVideoStreamMask)
                        {
                            free(node.pData);
                            node.pData = NULL;
                            goto prefetch_again;
                        ...
                        if(packet.infoVersion != nVideoInfoVersion && packet.info)
                        {
                            logd("cache -- video stream info change: "
                                    ...
                                    nVideoInfoVersion, packet.infoVersion);
                            struct VideoInfo *info = (struct VideoInfo *)malloc(sizeof(*info));
                    ...
                    {
                        if ((1 << node.nStreamIndex) & demux->mediaInfo.nAudioStreamMask)
                        {
                            free(node.pData);
                            node.pData = NULL;
                            goto prefetch_again;
                        ...
                        if(packet.infoVersion != nAudioInfoVersion && packet.info)
                        {
                            ...
                            struct AudioInfo *info = (struct AudioInfo *)malloc(sizeof(*info));
                            if(!info)
                            {
                                logd("malloc fail.");
                                return NULL;
...
static int setDataSourceFields(DemuxCompContext *demuxDhr, CdxDataSourceT* source,
                void* pHTTPServer, char* uri, KeyedVector<String8,String8>* pHeaders)
#else
static int setDataSourceFields(DemuxCompContext *demuxDhr, CdxDataSourceT* source,
                char* uri, KeyedVector<String8,String8>* pHeaders)
#endif
{
    CdxHttpHeaderFieldsT* pHttpHeaders;
    int                   i;
                ...
                !strncasecmp("https://", uri, 8)))
    {
        ...
        if(nHeaderSize > 0)
        {
            ...
            size_t size = sizeof(CdxHttpHeaderFieldT) * nHeaderSize;
            pHttpHeaders->pHttpHeader = (CdxHttpHeaderFieldT*)malloc(size);
    ...
    else if (pHeaders != NULL && strncasecmp("rtsp://", uri, 7) == 0)
    {
        ...
        for (i = 0; i < nHeaderSize; i++)
        {
            key = pHeaders->keyAt(i);
            value = pHeaders->valueAt(i);
            if (CdxKeyedVectorAdd(rtspHeader, key.string(), value.string()) != 0)
            {
                clearDataSourceFields(source);
                return -1;
            }
...
static int setMediaInfo(MediaInfo* pMediaInfo, CdxMediaInfoT* pInfoFromParser)
{
    ...
    if(pInfoFromParser->bitrate > 0)
        ...
        pMediaInfo->nBitrate = (int)((pInfoFromParser->fileSize*8*1000) /
                pInfoFromParser->program[0].duration);
    else
    ...
    pMediaInfo->nVideoStreamMask = pInfoFromParser->program[0].videoIndexMask;
    nMaskOutStreamNum = bitCount(pMediaInfo->nVideoStreamMask);
    if (nMaskOutStreamNum >= nStreamCount)
    {
        logw("%d video stream is forbidden", nStreamCount);
        nStreamCount = 0;
    }
    if(nStreamCount > 0)
    {
    ...
    pMediaInfo->nAudioStreamMask = pInfoFromParser->program[0].audioIndexMask;
    nMaskOutStreamNum = bitCount(pMediaInfo->nAudioStreamMask);
    if (nMaskOutStreamNum >= nStreamCount)
    {
        logw("%d audio stream is forbidden", nStreamCount);
        nStreamCount = 0;
    }
...
static int PlayerBufferOverflow(Player* p)
{
    ...
    if(PlayerHasVideo(p))
    {
        ...
        logv("picNum = %d, frameDuration = %d, dataSize = %d, bitrate = %d, "
                "bVideoOverflow = %d ,nStreamDataSize = %d ,nStreamBufferSize = %d",
    ...
    if(PlayerHasAudio(p))
    {
        ...
        logv("nPcmDataSize %d, nStreamDataSize %d, nBitrate %d, "
                "nAudioCacheTime %lld, bAudioOverflow %d",
            nPcmDataSize, nStreamDataSize, nBitrate,
            nAudioCacheTime, bAudioOverflow);
    }
    ...
    switch(eMessageId)
    {
        case 1: //YUNOS_TS_INFO
        {
            ...
            if(demux->pParser && param != NULL)
            {
                ...
                CdxParserControl(demux->pParser, CDX_PSR_CMD_GET_TS_M3U_BANDWIDTH,
                        (void*)&tsBandwidth);
                ...
                logv("TSInfoYUNOS IP[%s] Bandwidth[%lld] tsNum[%d] Duration[%lld] tsLength[%lld]",
                    demux->tsHttpTcpIP, demux->tsBandwidth, demux->tsNum,
                    demux->tsDuration, demux->tsLength);
        ...
        case 3: //YUNOS_HTTP_DOWNLOAD_ERROR_INFO
        {
            ...
            if( param != NULL && demux->pParser )
            {
                ...
                if( demux->httpRes[0])
                {
                    ...
                    if(httpResLen > 4096)
                        return -1;
                    base64Encode((unsigned char*)demux->httpRes, httpResLen,
                            (unsigned char*)base64EncodeOut);
        ...
        case 4:  //YUNOS_SOURCE set UUID
        {
            ...
            if(demux->pParser)
            {
                CdxParserControl(demux->pParser, CDX_PSR_CMD_SET_YUNOS_UUID,
                        (void*)demux->mYunOSUUID);
            }
            pthread_mutex_unlock(&demux->mutex1);
            break;
        }
...
static int base64Encode(const unsigned char *in_ptr, unsigned long encode_len,
                        unsigned char *out_ptr)
{
    const char *codes_src = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
