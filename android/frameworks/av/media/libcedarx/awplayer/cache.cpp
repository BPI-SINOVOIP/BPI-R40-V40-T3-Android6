 ...
 * File : cache.cpp
 *
 * Description : buffer for streaming media
 * History :
 *
 */
...
StreamCache* StreamCacheCreate(void)
{
    ...
    c->nLastValidPts     = -1;
    c->nLastValidPcr     = -1;
    c->nMaxBufferSize = 10 << 20;
    c->nStartPlaySize = 1024;
    pthread_mutex_init(&c->mutex, NULL);
    return c;
}

static void NodeInfoDestroy(CacheNode *node, bool freeCodecSpecific)
{
    if (node->info == NULL)
        return;
    ...
    /* If CodecSpecificData is already freed in other place and set to NULL, we
     * can free it again safely, and remove the check of freeCodecSpecific.
     * However, I'm not sure about other place. So keep the code as it is and
     * pray there is no memory leak.
     */
    if (freeCodecSpecific)
    {
        if(node->eMediaType == CDX_MEDIA_VIDEO)
        {
...
int StreamCacheOverflow(StreamCache* c)
{
    int bOverFlow;
    pthread_mutex_lock(&c->mutex);
    if(c->nDataSize >= c->nMaxBufferSize)
        bOverFlow = 1;
    else
        bOverFlow = 0;
    pthread_mutex_unlock(&c->mutex);
...
int StreamCacheDataEnough(StreamCache* c)
{
    int bDataEnough;
    pthread_mutex_lock(&c->mutex);
    if(c->nDataSize >= c->nStartPlaySize && c->nFrameNum > 0)
        bDataEnough = 1;
    else
        bDataEnough = 0;
    pthread_mutex_unlock(&c->mutex);
...
int StreamCacheAddOneFrame(StreamCache* c, CacheNode* node)
{
    ...
    newNode->nDuration      = node->nDuration;
    newNode->pNext          = NULL;
...
static int64_t StreamCacheSeekByPcr(StreamCache* c, int64_t nSeekTimeUs)
{
    ...
    if (pFirstNodeAfter != NULL &&
            llabs(nFirstPcrAfter - nSeekTimeUs) < llabs(nSeekTimeUs - nFoundMappedPts))
    {
     ...
     */
    if(pFirstNodeAfter == NULL &&
            llabs(nSeekTimeUs - nFoundMappedPts) > 3000000)
    {
...
static int StreamCacheSeekByOffset(StreamCache* c, int nSeekOffset)
{
    ...
    if(nSeekOffset < 0)
    {
        ...
        else
        {
            ...
            c->pPassedTail = pNode;
            c->pHead = pNodeFound;
            c->nDataSize += (c->nPassedDataSize - nPassedDataSize);
...
static int StreamCacheIsKeyFrame(StreamCache* c, CacheNode* pNode)
{
    ...
    else
    {
           ...
           c->eContainerFormat == CDX_PARSER_HLS)
        {
            ...
            else if(c->eVideoCodecFormat == VIDEO_CODEC_FORMAT_MPEG2 ||
                    c->eVideoCodecFormat == VIDEO_CODEC_FORMAT_MPEG1)
            {
