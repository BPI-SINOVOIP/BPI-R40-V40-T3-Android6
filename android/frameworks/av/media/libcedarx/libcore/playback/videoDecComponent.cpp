...
int VideoDecCompConfigHorizonScaleDownRatio(VideoDecComp* p, int nScaleDownRatio)
{
    return ConfigHorizonScaleDownRatio(p->pDecoder, nScaleDownRatio);
}

int VideoDecCompConfigVerticalScaleDownRatio(VideoDecComp* p, int nScaleDownRatio)
{
    return ConfigVerticalScaleDownRatio(p->pDecoder, nScaleDownRatio);
}

int VideoDecCompConfigRotation(VideoDecComp* p, int nRotateDegree)
{
    return ConfigRotation(p->pDecoder, nRotateDegree);
}

int VideoDecCompConfigDeinterlace(VideoDecComp* p, int bDeinterlace)
{
    return ConfigDeinterlace(p->pDecoder, bDeinterlace);
}

int VideoDecCompConfigThumbnailMode(VideoDecComp* p, int bOpenThumbnailMode)
{
    return ConfigThumbnailMode(p->pDecoder, bOpenThumbnailMode);
}

int VideoDecCompConfigOutputPicturePixelFormat(VideoDecComp* p, int ePixelFormat)
{
    return ConfigOutputPicturePixelFormat(p->pDecoder, ePixelFormat);
}

int VideoDecCompConfigNoBFrame(VideoDecComp* p, int bNoBFrames)
{
    return ConfigNoBFrames(p->pDecoder, bNoBFrames);
}

int VideoDecCompConfigDisable3D(VideoDecComp* p, int bDisable3D)
{
    return ConfigDisable3D(p->pDecoder, bDisable3D);
}

int VideoDecCompSetMemortThresh(VideoDecComp* p, int nMemoryThresh)
{
