...
static void CheckScreenRotateAngle(VideoRenderComp* p,
...
int VideoRenderCompSetWindow(VideoRenderComp* p, void* pNativeWindow)
{
    Message msg;
...
int VideoRenderCompSetVideoStreamInfo(VideoRenderComp* p, VideoStreamInfo* pStreamInfo)
{
...
int VideoRenderCompSetSyncFirstPictureFlag(VideoRenderComp* p, int bSyncFirstPictureFlag)
{
...
process_message:
            ...
            if(pLayerBuffer != NULL)
            {
                if(p->pLayerCtrl != NULL)
                {
                    if(pSecondLayerBufferOf3DMode == NULL)
                        ...
                        p->mLayerOps->queue3DBuffer(p->pLayerCtrl, pLayerBuffer,
                                pSecondLayerBufferOf3DMode, 0);
                }
                pLayerBuffer = NULL;
                pSecondLayerBufferOf3DMode = NULL;
            }
            ...
            if(pPreLayerBuffer != NULL)
            {
                if(p->pLayerCtrl != NULL)
                {
                    if(pSecondLayerBufferOf3DMode == NULL)
                        ...
                        p->mLayerOps->queue3DBuffer(p->pLayerCtrl, pPreLayerBuffer,
                                pSecondLayerBufferOf3DMode, 0);
                }
                pPreLayerBuffer = NULL;
                pSecondLayerBufferOf3DMode = NULL;
        ...
        else if(msg.messageId == MESSAGE_ID_SET_WINDOW)
        {
...
#endif
            {
                ...
                if(p->pLayerCtrl != NULL)
                    p->mLayerOps->setCallback(p->pLayerCtrl, (PlayerCallback)LayerCallback,
                            (void*)p);
                else
                {
        ...
        else if(msg.messageId == MESSAGE_ID_SET_3D_MODE)
        {
            ...
            if(p->pLayerCtrl != NULL)
            {
                ...
                *pReplyValue = LayerSetDisplay3DMode(p->pLayerCtrl,
                        (enum EDISPLAY3DMODE)msg.params[3]);
            }
            else
            {
        ...
        else if(msg.messageId == MESSAGE_ID_SET_HOLD_LAST_PICTURE)
        {
            ...
            logv("process MESSAGE_ID_SET_HOLD_LAST_PICTURE message, bHoldLastPicture(%d)",
                    bHoldLastPicture);
            sem_post(pReplySem);
        ...
        else if(msg.messageId == MESSAGE_ID_RENDER)
        {
            ...
            if(pPicture == NULL && pLayerBuffer == NULL)
            {
                    ...
                    while(pPicture == NULL)
                    {
                        ...
                        if(bResolutionChange)
                        {
                            ...
                            if(pPreLayerBuffer != NULL)
                            {
                                ...
                                {
                                    if(pSecondLayerBufferOf3DMode == NULL)
                                        p->mLayerOps->queueBuffer(p->pLayerCtrl,
                                                pPreLayerBuffer, 0);
                                    else
                                        p->mLayerOps->queue3DBuffer(p->pLayerCtrl,
                                                pPreLayerBuffer, pSecondLayerBufferOf3DMode, 0);
                                }
                                pPreLayerBuffer = NULL;
                                pSecondLayerBufferOf3DMode = NULL;
                            }
                            ...
                            if(p->pLayerCtrl != NULL)
                            {
                                ...
                                if(p->pLayerCtrl != NULL)
                                {
                                    p->mLayerOps->setCallback(p->pLayerCtrl,
                                            (PlayerCallback)LayerCallback, (void*)p);
                                    bNeedResetLayerParams = 1;
                                }
                            }
                ...
                if(bFirstPictureShowed == 0 || bNeedResetLayerParams == 1)
                {
                    ...
                    logv("pPicture width(%d) height(%d)", pPicture->nWidth, pPicture->nHeight);
                    logv("width %d, height %d",
                            p->videoStreamInfo.nWidth, p->videoStreamInfo.nHeight);
                    #if(NOT_DROP_FRAME == 1)
                       ...
                       (pPicture->nBottomOffset - pPicture->nTopOffset) > 0)
                    {
                        ...
                        p->callback(p->pUserData, PLAYER_VIDEO_RENDER_NOTIFY_VIDEO_CROP,
                                (void*)size);
                    }
                    ...
                    if(p->pLayerCtrl)
                    {
                        ...
                        p->mLayerOps->setDisplayBufferSize(p->pLayerCtrl, pPicture->nWidth,
                                pPicture->nHeight);
                        ...
                        p->mLayerOps->setDisplayPixelFormat(p->pLayerCtrl,
                                (enum EPIXELFORMAT)display_pixel_format);
                ...
                /***********************************************************
                 * 4. notify the first sync frame to set timer. the first sync
...
step_4:
                    ...
                    || (bFirstPictureShowed == 0 && p->bSyncFirstPictureFlag == 1))
                {
                    if(bFirstPtsNotified == 0)
                    {
                         ...
                         */
                        ret = p->callback(p->pUserData, PLAYER_VIDEO_RENDER_NOTIFY_FIRST_PICTURE,
                                (void*)&pPicture->nPts);
                        ...
                        {
                            /* waiting process for first frame sync with audio
                             * is broken by a new message to player, so the
                             * player tell us to notify again later.  post a
                             * render message to continue the rendering job
                ...
                //******************************************************
step_5:
                logv("nDeinterlaceDispNum %d, bDeinterlaceFlag %d",
                        nDeinterlaceDispNum, bDeinterlaceFlag);
                for(int nDeinterlaceTime = 0; nDeinterlaceTime < nDeinterlaceDispNum;
                        nDeinterlaceTime++)
                {
                    if(pLayerBuffer == NULL)
                    {
                            ...
                            if(p->pLayerCtrl != NULL)
                            {
                                while(pLayerBuffer == NULL)
                                {
                                    ...
                                    if(pPreLayerBuffer != NULL)
                                    {
                                        ...
                                        nLayerBufferMode =
                                            p->mLayerOps->dequeueBuffer(p->pLayerCtrl,
                                                    &pLayerBuffer);
                                    ...
                                    else if(nLayerBufferMode == 0)
                                    {
                                        if(bDeinterlaceFlag == 1)
                                        {
                                             ...
                                             * bot filed or top filed error, cannot display both of
                                             * them, Why!!!!
                                             */
                                            if(p->pDeinterlacePrePicture == pPicture &&
                                                    (bTopFiledError || bBotFiledError))
                                            {
                                                bTopFiledError = bBotFiledError = 1;
                                            }
                                            logv("p->pDeinterlacePrePicture: %p, pPicture: %p",
                                                    p->pDeinterlacePrePicture, pPicture);
                                            ...
                                            if (diret != 0)
                                            {
                                                ...
                                                //          a) cancel to layer
                                                //          b) set to pPreLayerBuffer for next
                                                //          deinterlace process.
                                                //      we use b).
                                                if (nDeinterlaceTime == (nDeinterlaceDispNum - 1)) {
                                                    ...
                                                    if(pPicture != p->pDeinterlacePrePicture)
                                                        VideoDecCompReturnPicture(p->pDecComp,
                                                                p->pDeinterlacePrePicture);
                                                    p->pDeinterlacePrePicture = pPicture;
                                            ...
                                            // iv) end of deinterlacing this frame, set prepicture
                                            // for next frame.
                                            if(nDeinterlaceTime == (nDeinterlaceDispNum - 1))
                                            {
                                                if(pPicture != p->pDeinterlacePrePicture
                            ...
                            if(p->pLayerCtrl != NULL)
                            {
                                while(pLayerBuffer == NULL)
                                {
                                    if(pPreLayerBuffer != NULL)
                                    {
                                        ...
                                        nLayerBufferMode =
                                            p->mLayerOps->dequeue3DBuffer(p->pLayerCtrl,
                                                    &pLayerBuffer, &pSecondLayerBufferOf3DMode);
                    ...
                    logv("topFiledFirst:%d topfiledError: %d, pLayerBuffer->bBottomFieldError: %d",
                            bTopFiledFirst, bTopFiledError, bBotFiledError);
...
//step_6:
                       ...
                       || (bFirstPictureShowed == 0 && p->bSyncFirstPictureFlag == 1))
                    {
                        ...
                        nWaitTime = p->callback(p->pUserData,
                                PLAYER_VIDEO_RENDER_NOTIFY_PICTURE_PTS,
                                (void*)&pLayerBuffer->nPts);
                        ...
                        #if ((NATIVE_WIN_DISPLAY_CMD_GETDISPFPS == 1) && \
                        ...
                        {
                            int dispFPS = pNativeWindow->perform(pNativeWindow,
                                    NATIVE_WINDOW_GETPARAMETER, DISPLAY_CMD_GETDISPFPS);
                            int frameRate = 0;

                            if (p->pFrameRateEstimater)
                                frameRate =
                                    FramerateEstimaterGetFramerate(p->pFrameRateEstimater) / 1000;
                            else
                            ...
                            if (dispFPS > 0 && frameRate > dispFPS)
                                dropTime = -100;
                        }
                        #else
                            int dropTime = -2500;
                        ...
                        if(nWaitTime > 0)
                        {
                            ...
                            while(nWaitTime > 0)
                            {
                                ...
                                {
                                    /* if pLayerCtrl==null, we should return picture before
                                     * process message or the picture will never be returned.
                                     */
                                    if(p->pLayerCtrl == NULL)
                                    {
                                        if(bVideoWithTwoStream == 0)
                    ...
                    if(p->pLayerCtrl != NULL)
                    {
                        ...
                        if ((p->pAvTimer != NULL)
                            && (SEND_PTS_TO_SF == 1))
                        {
                            int64_t ptsAbs = p->pAvTimer->PtsToSystemTime(p->pAvTimer,
                                    pLayerBuffer->nPts);
                                   ...
                                   int bNeedResetAngleFlag)
{
    if(p->pLayerCtrl != NULL)
    {
        ...
        if(p->nRotationAngle != nCurRotation || bNeedResetAngleFlag == 1)
        {
            ...
            logv("nRotate %d rect info : w[%d], h[%d], l[%d], t[%d], dw[%d], dh[%d], "
                    "realRrightO[%d], realBottom[%d]",
                  nCurRotation,
                  nRotateWidth,
                  nRotateHeight,
                  nRotateLeftOffset,
