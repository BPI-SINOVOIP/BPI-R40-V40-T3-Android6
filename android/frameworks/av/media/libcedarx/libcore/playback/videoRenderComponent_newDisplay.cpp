...
static inline void NotifyVideoSizeAndSetDisplayRegion(VideoRenderComp* p);

static inline int ProcessVideoSync(VideoRenderComp* p,
                                    VideoPicture* pPicture);
static inline int QueueBufferToShow(VideoRenderComp* p,
                                                VideoPicture* pPicture);
static inline int ProcessDeinterlace(VideoRenderComp* p,
                                              int            nDeinterlaceTime);
static inline int RenderGetVideoFbmBufInfo(VideoRenderComp* p);

static inline int SetGpuBufferToDecoder(VideoRenderComp *p);

static inline int ResetBufToDecoder(VideoRenderComp *p);
...
VideoRenderComp* VideoRenderCompCreate(void)
{
    ...
    p->mVideoScheduler = CdxVideoSchedulerCreate();
    if (p->mVideoScheduler == NULL)
...
int VideoRenderCompDestroy(VideoRenderComp* p)
{
    ...
    MessageQueueDestroy(p->mq);
    CdxVideoSchedulerDestroy(p->mVideoScheduler);
    free(p);
...
int VideoRenderCompReset(VideoRenderComp* p)
{
    ...
    CdxVideoSchedulerRestart(p->mVideoScheduler);
...
process_message:
        ...
        else if(msg.messageId == MESSAGE_ID_SET_WINDOW)
        {
            ...
            if(p->bHadSetLayerInfoFlag == 0 && p->mFbmBufInfo.nBufNum != 0)
            {
               ...
               p->mNewLayerOps->setDisplayPixelFormat(p->pLayerCtrl,
                       (enum EPIXELFORMAT)pFbmBufInfo->ePixelFormat);
               p->mNewLayerOps->setVideoWithTwoStreamFlag(p->pLayerCtrl,
                       p->bVideoWithTwoStream);
               p->mNewLayerOps->setIsSoftDecoderFlag(p->pLayerCtrl,
                       pFbmBufInfo->bIsSoftDecoderFlag);
               p->mNewLayerOps->setDisplayBufferSize(p->pLayerCtrl,
                       pFbmBufInfo->nBufWidth, pFbmBufInfo->nBufHeight);
               p->nGpuBufferNum = p->mNewLayerOps->setDisplayBufferCount(p->pLayerCtrl,
                       pFbmBufInfo->nBufNum);
                ...
                while(p->pPicture == NULL)
                {
                    ...
                    if(p->bResolutionChange)
                    {
                        ...
                        if(p->pLayerCtrl != NULL)
                        {
                            ...
                            p->pLayerCtrl = p->mNewLayerOps->init(p->pNativeWindow,
                                    p->bProtectedBufferFlag);
                            if(p->pLayerCtrl != NULL)
                                p->bNeedResetLayerParams = 1;
                        }
...
step_5:
                ...
                if(p->bDeinterlaceFlag == 0)
                {
                    ...
                    if(p->bFirstPictureShowed != 0)
                    {
                        ret = ProcessVideoSync(p, p->pPicture);
                ...
                {
                    for(int nDeinterlaceTime = 0; nDeinterlaceTime < p->nDeinterlaceDispNum;
                            nDeinterlaceTime++)
                    {
                                    ...
                                    VideoPicture* pPicture)
{
    ...
    if(nWaitTime > 0)
    {
        ...
        while(nWaitTime > 0)
        {
            ...
            ret = MessageQueueWaitMessage(p->mq, nWaitTimeOnce);
                                                ...
                                                VideoPicture* pPicture)
{
    if(p->pLayerCtrl != NULL)
    {
        if (SEND_PTS_TO_SF == 1)
        {
            ...
            int64_t renderTime = CdxVideoSchedulerSchedule(p->mVideoScheduler, ptsAbs);
            logv("ptsAbs %f ms, renderTime %f ms   %f ms",
                    ptsAbs / 1000000.0, renderTime / 1000000.0, (renderTime - p->pre)/ 1000000.0);
            p->pre = renderTime;

            //attention! 4 vsync period is set after measured in h5(video before audio),
            //different platform may has different value.
            int nVsync = GetConfigParamterInt("compensate_vsync", 4);
            renderTime += nVsync * CdxVideoSchedulerGetVsyncPeriod(p->mVideoScheduler);
            p->mNewLayerOps->setBufferTimeStamp(p->pLayerCtrl, renderTime);
        }
...
static inline int RenderGetVideoFbmBufInfo(VideoRenderComp* p)
{
    ...
    if(pFbmBufInfo != NULL)
    {
        ...
        if(p->pLayerCtrl != NULL)
        {
            ...
            p->mNewLayerOps->setDisplayBufferSize(p->pLayerCtrl, pFbmBufInfo->nBufWidth,
