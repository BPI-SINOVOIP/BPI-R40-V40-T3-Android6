...
//******************************************************************//
   CedarXSubRender::CedarXSubRender()
   {
      ...
      logd("CedarXSubRender::CedarXSubRender xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx1");
        mUserSetFontColor = 0;
        mUserSetFontSize  = 20;
        mUserSetFontStyle = 0;
        mUserSetYPercent  = 10;
      ...
      logd("CedarXSubRender::CedarXSubRender xxxxxxxxxxxxxxxxxxxxxxxxxxxx2");
...
/*
      ...
      num ++;
      FILE *fp = fopen(fname, "w");
      fwrite((void *)outBuffer.bits,
             outBuffer.width * outBuffer.height * bytesPerPixel(outBuffer.format), 1, fp);
      fflush(fp);
      fclose(fp);
*/
       mCanvas->setBitmapDevice(mBitmap);
  ...
  //*********************************************************** //
    ...
    int textBoxStartx,int textBoxEndx, int textBoxStarty, int textBoxEndy, int specialEffectFlag)
    {
       ...
       {
            nCount = countLines(text, textStop - text, paint, marginWidth, subTextWidth,
                 specialEffectFlag);
            SkASSERT(nCount > 0);
