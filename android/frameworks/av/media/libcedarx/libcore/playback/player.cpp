...
int PlayerConfigSetMemoryThresh(Player* pl, int nMemoryThresh)
{
    ...
    if(p->pVideoDecComp != NULL)
    {
        ...
        return VideoDecCompSetMemortThresh(p->pVideoDecComp, nMemoryThresh);
    }
    return 0;
}
...
#if (ENABLE_SUBTITLE_DISPLAY_IN_CEDARX == 1)
    SubtitleRenderCompSetRenderInner(p->pSubtitleRender, p->bCmccAppPlayer);
#endif
    return 0;
}
