...
static void FlushExpiredItems(SubtitleRenderCompContext* p,
...
#if ((ENABLE_SUBTITLE_DISPLAY_IN_CEDARX == 0) && \
    (CONFIG_ALI_YUNOS == OPTION_ALI_YUNOS_NO || \
     CONFIG_OS_VERSION != OPTION_OS_VERSION_ANDROID_4_4))
#define SubRenderCreate()       abort()
#define SubRenderDestory()      abort()
#define SubRenderHide(x, y)
#define SubRenderShow()
#define SubRenderDraw(x)
#endif
                                         ...
                                         int                  nDefaultStreamIndex)
{
    ...
    if(p->pStreamInfo[p->nStreamSelected].eCodecFormat == SUBTITLE_CODEC_IDXSUB ||
        p->pStreamInfo[p->nStreamSelected].eCodecFormat == SUBTITLE_CODEC_PGS)
    {
        p->bRenderInner = true;
        logd("This subtitle will render inner.");
    }
...
int SubtitleRenderCompSetRenderInner(SubtitleRenderComp* s, bool bRenderInner)
{
    SubtitleRenderCompContext* p;
    p = (SubtitleRenderCompContext*)s;

    p->bRenderInner = bRenderInner;
    return 0;
}
#endif
            ...
            else
            {
                ...
                else
                {
                    ...
                    if(p->bRenderInner)
                    {
                        ...
                        dispSubItem.subTextBuf = (unsigned char*)pCurItem->pText;
                        dispSubItem.subTextLen = (unsigned int)pCurItem->nTextLength;
                        dispSubItem.encodingType = pCurItem->eTextFormat;
                    ...
                    else
                    {
                        ...
                        p->callback(p->pUserData, PLAYER_SUBTITLE_RENDER_NOTIFY_ITEM_AVAILABLE,
                                (void*)callbackParam);
                    }
...
static void FlushExpiredItems(SubtitleRenderCompContext* p, SubtitleItemInfo* pItemInfo,
        int64_t nTimeDelayUs)
{
    ...
    for(i=0; i<MAX_SUBTITLE_ITEM; i++)
    {
        ...
        if(ret < 0)
        {
            ...
            {
                p->callback(p->pUserData, PLAYER_SUBTITLE_RENDER_NOTIFY_ITEM_EXPIRED,
                        (void*)(&pItemInfo[i].nId));
            }
        }
    }
