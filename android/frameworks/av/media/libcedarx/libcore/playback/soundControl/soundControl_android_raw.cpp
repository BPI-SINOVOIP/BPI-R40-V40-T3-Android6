 ...
 * All rights reserved.
 *
 * File : soundControl_android_raw.cpp
 * Description : soundControl android for raw data
 * History :
 *
 */
...
static int SoundDeviceSetRawFlag(int card, int raw_flag)
{
    ...
    const char *control_name = (card == 1) ?
            "hdmi audio format Function" : "spdif audio format Function";
    const char *control_value = (raw_flag==SND_AUDIO_RAW_DATA_AC3) ?
                                "AC3" : (raw_flag==SND_AUDIO_RAW_DATA_DTS) ?
                                "DTS" :(raw_flag==SND_AUDIO_RAW_DATA_DOLBY_DIGITAL_PLUS)?
                                "DOLBY_DIGITAL_PLUS":"pcm";
...
#ifdef CDX_SUPPROT_IEC61937
        if((sc->raw_data.nRawDataFlag == SND_HDMI_RAW) &&
           (sc->config.raw_flag == SND_AUDIO_RAW_DATA_AC3 ||
            sc->config.raw_flag == SND_AUDIO_RAW_DATA_DTS))
        {
