...
* All rights reserved.
*
* File : layerControl_linux.cpp
* Description : surface display interface -- for linux
* History :
*/
...
#endif
//* use layer 1 to show video, totally 4 layers, gui generally use layer 0.
static const int DISPLAY_LAYER = 1;

static VideoPicture* gLastPicture = NULL;

#if 0
/* NUM_OF_PICTURES_KEEP_IN_LIST has defined in config.h */
//* it may be set to 3 if new version of display system used.
...
int LayerSetDisplayRegion(LayerCtrl* l, int nLeftOff, int nTopOff,
        ...
        while(i > NUM_OF_PICTURES_KEEP_IN_LIST)
        {
            ...
            if(nodePtr->pSecondPictureOf3D != NULL)
            {
                lc->callback(lc->pUserData, MESSAGE_ID_LAYER_RETURN_BUFFER,
                             (void*)nodePtr->pSecondPictureOf3D);
...
static int SetLayerParam(LayerCtrlContext* lc)
{
    ...
    switch(lc->eReceivePixelFormat)
    {
        ...
        case PIXEL_FORMAT_YV12:
            //* can not find a pixel format for YV12 in drv_display.h, use YUV planer.
            pixelFormat = DISP_FORMAT_YUV420_P;
            break;
                                   ...
                                   VideoPicture*     pSecondPictureOf3D)
{
    ...
    {
        pLayerInfo->id                   = pPicture->nID;
        pLayerInfo->fb.addr[0]
            = (unsigned long)MemAdapterGetPhysicAddressCpu(pPicture->pData0);
        pLayerInfo->fb.addr[1]
            = (unsigned long)MemAdapterGetPhysicAddressCpu(pPicture->pData1);
        pLayerInfo->fb.trd_right_addr[0]
            = (unsigned long)MemAdapterGetPhysicAddressCpu(pSecondPictureOf3D->pData0);
        pLayerInfo->fb.trd_right_addr[1]
            = (unsigned long)MemAdapterGetPhysicAddressCpu(pSecondPictureOf3D->pData1);
        //* should we set the address fb.addr[2] according to the pixel format?
    }
