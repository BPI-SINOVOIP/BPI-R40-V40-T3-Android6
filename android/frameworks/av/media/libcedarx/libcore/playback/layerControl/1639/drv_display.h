...
typedef struct {unsigned char  alpha;unsigned char red;unsigned char green;
                unsigned char blue; }disp_color_info;
...
typedef enum ___disp_pixel_format
{
...
typedef enum ___disp_3d_src_mode
{
...
typedef enum ___disp_3d_out_mode
{
...
typedef enum ___disp_color_range
{
...
typedef enum ___disp_output_type
{
...
typedef enum ___disp_out_csc_type
{
...
typedef enum ___disp_cursor_mode
{
...
typedef enum ___disp_lcd_hv_if
{
...
typedef enum ___disp_lcd_hv_srgb_seq
{
...
typedef enum ___disp_lcd_hv_syuv_seq
{
...
typedef enum ___disp_lcd_hv_syuv_fdly
{
...
typedef enum ___disp_lcd_cpu_if
{
...
typedef enum ___disp_lcd_lvds_if
{
    LCD_LVDS_IF_SINGLE_LINK        = 0,
    LCD_LVDS_IF_DUAL_LINK          = 1,
}disp_lcd_lvds_if;

typedef enum ___disp_lcd_lvds_colordepth
{
    LCD_LVDS_8bit        = 0,
    LCD_LVDS_6bit        = 1,
}disp_lcd_lvds_colordepth;

typedef enum ___disp_lcd_lvds_mode
{
    LCD_LVDS_MODE_NS          = 0,
    LCD_LVDS_MODE_JEIDA        = 1,
}disp_lcd_lvds_mode;

typedef enum ___disp_lcd_dsi_if
{
...
typedef enum ___disp_lcd_dsi_lane
{
    LCD_DSI_1LANE            = 1,
    LCD_DSI_2LANE            = 2,
    LCD_DSI_3LANE            = 3,
...
typedef enum ___disp_lcd_dsi_format
{
...
typedef enum ___disp_lcd_cmap_color
{
    LCD_CMAP_B0    = 0x0,
    LCD_CMAP_G0    = 0x1,
    LCD_CMAP_R0    = 0x2,
...
typedef struct ___disp_fb_info
{
...
// framebuffer source window,only take x,y if is not scaler mode
    disp_window           src_win;
#if 0
    bool                  interlace;
...
typedef struct ___disp_layer_info
{
    disp_layer_mode           mode;       //layer work mode
    unsigned char             pipe;       //layer pipe,0/1
    //layer priority,can get layer prio,but never set layer prio,从底至顶,优先级由低至高
    ...
    //get the id of the frame display now by DISP_CMD_LAYER_GET_FRAME_ID
    unsigned int              id;
}disp_layer_info;

typedef struct ___disp_colorkey
{
...
typedef struct disp_capture_para
{
    //used when the screen is not displaying on any output device(lcd/hdmi/vga/tv)
...
typedef struct __disp_dsi_dphy_timing_t
{
...
typedef struct ___panel_extend_para
{
...
typedef struct ___disp_panel_para
{
    disp_lcd_if              lcd_if;
...
typedef struct ___disp_video_timing
{
...
typedef struct ___disp_lcd_panel_fun
{
    void (*cfg_panel_info)(panel_extend_para * info);
    int (*cfg_open_flow)(unsigned int sel);
    int (*cfg_close_flow)(unsigned int sel);
    int (*lcd_user_defined_func)(unsigned int sel,
                                 unsigned int para1, unsigned int para2, unsigned int para3);
}disp_lcd_panel_fun;
...
typedef struct ___disp_fb_create_info
{
...
typedef enum ___disp_init_mode
{
    ...
    //fb0 for two different screen(screen0 layer is normal layer, screen1 layer is scaler layer);
    DISP_INIT_MODE_TWO_DIFF_SCREEN_SAME_CONTENTS = 4,
}disp_init_mode;

typedef struct ___disp_init_para
{
    bool                  b_init;
...
typedef struct ___setup_dispc_data_t
{
    int                 post2_layers;
