...
* All rights reserved.
*
* File : layerControl_android.cpp
* Description : surface display interface
* History :
*/
...
void checkGpuOutputType(LayerCtrl* l)
{
    ...
    int nCurOutputType = lc->pNativeWindow->perform(lc->pNativeWindow,
                                                    NATIVE_WINDOW_GET_OUTPUT_TYPE);
...
static void updateKeepListBuffer(LayerCtrlContext* lc, buffer_handle_t handle,
                                 VideoPicture* pBuf0, VideoPicture* pBuf1)
{
    int i = 0;
    ...
    for(i = 0; i < NUM_OF_PICTURES_KEEP_IN_LIST; i++)
    {
            ...
            && (lc->pKeepPicNode[i].pPicture != NULL))
        {
            ...
            lc->callback(lc->pUserData, MESSAGE_ID_LAYER_RETURN_BUFFER,
                         (void*)lc->pKeepPicNode[i].pPicture);
            if (lc->pKeepPicNode[i].pSecondPictureOf3D != NULL)
            {
                lc->callback(lc->pUserData, MESSAGE_ID_LAYER_RETURN_BUFFER,
                             (void*)lc->pKeepPicNode[i].pSecondPictureOf3D);
            }
...
static int initLayerParamNormal(LayerCtrlContext* lc)
{
    ...
    {
        logw("original picture align_width[%d], height[%d] mod8 = %d",
            nGpuBufWidth, nGpuBufHeight, nGpuBufHeight%8);
    ...
    native_window_set_buffers_geometry(lc->pNativeWindow,
                                       nGpuBufWidth, nGpuBufHeight, pixelFormat);
    lc->pNativeWindow->perform(lc->pNativeWindow, NATIVE_WINDOW_SET_CROP, &crop);
    /* native_window_set_buffer_count() will free buffers insides,
       so do not do it again when seek */
...
int queueBufferNormal(LayerCtrlContext* lc, VideoPicture* pBuf, int bValid)
{
    ...
    //* unlock the buffer.
    {
     ...
     nBufParam[0], nBufParam[1],nBufParam[2], nBufParam[3],
     nBufParam[4], nBufParam[5], nBufParam[6]);
#endif
...
static int sendThreeBlackFrameToGpu(LayerCtrlContext* lc)
{
    ...
    for(i = 0;i < NUM_OF_PICTURES_KEEP_IN_LIST;i++)
    {
        ...
        lc->pNativeWindow->perform(lc->pNativeWindow, NATIVE_WINDOW_SET_VIDEO_BUFFERS_INFO,
                                   nBufAddr[0], nBufAddr[1],nBufAddr[2],
                                   nBufAddr[3], nBufAddr[4], nBufAddr[5], nBufAddr[6]);
                                   ...
                                   VideoPicture*      pSecondPictureOf3D)
{
    ...
    if(pSecondPictureOf3D == NULL)
    {
        ...
        pHwcLayerPictureInfo->flag_addr
            = (unsigned long)CdcMemGetPhysicAddressCpu(lc->memops, pPicture->pMafData);
        ...
        pHwcLayerPictureInfo->top_y
            = (unsigned long)CdcMemGetPhysicAddressCpu(lc->memops, pPicture->pData0);
        pHwcLayerPictureInfo->top_c
            = (unsigned long)CdcMemGetPhysicAddressCpu(lc->memops, pPicture->pData1);
        pHwcLayerPictureInfo->bottom_y
            = (unsigned long)CdcMemGetPhysicAddressCpu(lc->memops, pPicture->pData2);
        pHwcLayerPictureInfo->bottom_c
            = (unsigned long)CdcMemGetPhysicAddressCpu(lc->memops, pPicture->pData3);
    }
    else
    {
        pHwcLayerPictureInfo->number          = pPicture->nID;
        ...
        pHwcLayerPictureInfo->top_c
            = (unsigned long)CdcMemGetPhysicAddressCpu(lc->memops, pPicture->pData1);
        pHwcLayerPictureInfo->bottom_y
            = (unsigned long)CdcMemGetPhysicAddressCpu(lc->memops, pSecondPictureOf3D->pData0);
        pHwcLayerPictureInfo->bottom_c
            = (unsigned long)CdcMemGetPhysicAddressCpu(lc->memops, pSecondPictureOf3D->pData1);
    }
                                      ...
                                      int bValid)
{
    ...
    if(lc->bLayerShowed == 1)
    {
        int nCurFrameId;
        int nWaitTime;
        //* max frame interval is 1000/24fps = 41.67ms, here we wait 50ms for max.
        nWaitTime = 50000;
        do
        {
...
static void getVideoBufferInfo(int *nBufAddr, VideoPicture* pBuf0,
                               VIDEOPICTURE* pBuf1, struct ScMemOpsS *memops)
{
                                    ...
                                    int bValid)
{
    ...
    {
        //logd("******** GPU->0 COPY, send black frame to GPU!!! %d", lc->nOutputTypeChanged);
        memset((char*)pDataBuf,0x10,(pWindowBuf->height * pWindowBuf->stride));
        memset((char*)pDataBuf + pWindowBuf->height * pWindowBuf->stride,
                0x80,(pWindowBuf->height * pWindowBuf->stride)/2);
    ...
    lc->pNativeWindow->perform(lc->pNativeWindow, NATIVE_WINDOW_SET_DIT_INFO,
                               !pBuf0->bIsProgressive, pBuf0->bTopFieldFirst);
...
static int queueLastBufferZeroCpyNormal(LayerCtrlContext* lc, VideoPicture* pPicture)
{
    ...
    for(int k = 0;k < NUM_OF_PICTURES_KEEP_IN_LIST;k++)
    {
        ...
        mQueuePicture.pData1       = mQueuePicture.pData0 +
                                    (pWindowBuf->height * pWindowBuf->stride);
        mQueuePicture.pData2       = mQueuePicture.pData1 +
                                    (pWindowBuf->height * pWindowBuf->stride)/4;
...
int __LayerSetDisplayPixelFormat(LayerCtrl* l, enum EPIXELFORMAT ePixelFormat)
{
    ...
    else
    {
        ...
        //* add new pixel formats supported by hardware layer here.
    ...
    *ppBuf = NULL;
    ...
    else
    {
        ...
        if(lc->need_get_phy_addr_flag)
        {
            ...
            pPicture->pData1       = (char*)(nPhyaddress +
                                     (pWindowBuf->height * pWindowBuf->stride));
            pPicture->pData2       = (char*)(nPhyaddress +
                                     (pWindowBuf->height * pWindowBuf->stride)*5/4);
            pPicture->phyYBufAddr  = nPhyaddress - VE_PHY_OFFSET;
            pPicture->phyCBufAddr  = nPhyaddress +
                                    (pWindowBuf->height * pWindowBuf->stride) - VE_PHY_OFFSET;
        }
        else
        {
            pPicture->pData0       = (char*)pDataBuf;
            pPicture->pData1       = pPicture->pData0 +
                                     (pWindowBuf->height * pWindowBuf->stride);
            pPicture->pData2       = pPicture->pData1 +
                                     (pWindowBuf->height * pWindowBuf->stride)/4;
        }
