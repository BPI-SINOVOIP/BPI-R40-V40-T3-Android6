...
cdx_int32 ID3PsrGetLanguageEncoding(cdx_char* ulcomm)
{
    cdx_char* uLanguage = ulcomm;
    cdx_int32 ret = -1;
    cdx_int32 i;
...
cdx_int32    ID3PsrGetStringLen(cdx_char *StringPtr)
{
    cdx_uint32 Len = 0;
    while(*StringPtr++!=0)
    {
...
static void ID3PsrReadNunicode(cdx_int32 *FrameLen, cdx_char** Frame, Id3ParserImplS *id3,
                               int frameTLen)
{
    cdx_char data;
    cdx_int32 i;
    ...
    *Frame = id3->mInforBuf;//
    ...
    {
        data= ID3PsrGetBsInByte(1,id3,1);
        *id3->mInforBuf++= data;
    }
    return;
}

static void ID3PsrReadunicode(cdx_int32 *FrameLen, cdx_char** Frame, Id3ParserImplS *id3,
                              int frameTLen)
{
    cdx_int32 UnicodeBOM;
    cdx_char data,data1,data2;
    cdx_int32 i;
    if(frameTLen<4)
    {
        *FrameLen = 0;
    ...
    *Frame = id3->mInforBuf;//
    ...
    {
        data= ID3PsrGetBsInByte(1,id3,1);
        data1= ID3PsrGetBsInByte(1,id3,1);
...
cdx_int32    ID3PsrGeTID3_V2(Id3ParserImplS *parser)  //IF HAVE RETURN ID3 tag length
{
    ...
    if(BsVal == 0x494433/*ID3*/)  //ID3v2/file identifier   "ID3"
    {
        ...
        while((Id3v2len>=10)&&(frameinfoseg<ID3TAGNUM))
        {
            ...
            {
                cdx_char *GenreTemp = id3->mInforBuf;//for use AIF->mInforBuf buffer
                ...
                if(GenreTemp[0]=='(')
                {
                    ...
                    else
                    {
                        ...
                        if(GenreIndex<148+2)
                        {
                            ...
                            id3->mGenre_sz=ID3PsrGetStringLen(id3->mGenre);
                            id3->mInforBuf += 22;
                        }
                    }
                }
