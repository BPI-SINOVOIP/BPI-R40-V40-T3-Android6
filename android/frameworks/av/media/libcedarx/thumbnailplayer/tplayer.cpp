...
*
* File : xxx.c
* Description : Thumbnail video player
* History :
*
*/
...
status_t TPlayer::setDataSource(const sp<IMediaHTTPService> &httpService,const char* url,
...
status_t TPlayer::setSpeed(int mspeed)
{
    CEDARX_UNUSE(mspeed);
    return OK;
}
        ...
        if(t->mStatus == PLAYER_STATE_PLAYING)
        {
            ...
            if(ValidPictureNum(t->mDecoder,0))
            {
                ...
                if(firstFrame == 1)
                {
                    ...
                    native_window_set_buffers_geometry(t->mNativeWindow.get(), widthAlign,
                                                       heightAlign,HAL_PIXEL_FORMAT_YV12);
    ...
    if(ret != 0)
    {
        ...
        continue;
...

                {
                    ...
                    memcpy((char*)dst + picture->nWidth*picture->nHeight, (void*)picture->pData1,
            ...
            else
            {
                while(1)
                {
                    ...
                    //when hdmi plugin and plugout, thumbplayer will ...
                    logv("(f:%s l:%d) TPlayer", __FUNCTION__, __LINE__);
                    ...
                    else if(decodeResult == VDECODE_RESULT_NO_BITSTREAM)
                    {
                        ...
                        ret = RequestVideoStreamBuffer(t->mDecoder, require_size,
