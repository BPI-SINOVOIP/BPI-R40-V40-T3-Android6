...
VideoFrame *AwRetrieverGetFrameAtTime(AwRetriever* v, int64_t timeUs)
{
    ...
    if(p->mVideoDecoder == NULL)
    {
        ...
        if(vconfig.nRotateDegree == 1 || vconfig.nRotateDegree == 3)
        {
            ...
            if(p->mMediaInfo.program[p->mMediaInfo.programIndex].video[0].nWidth > 960 ||
                p->mMediaInfo.program[p->mMediaInfo.programIndex].video[0].nWidth  == 0)
            ...
            if(p->mMediaInfo.program[p->mMediaInfo.programIndex].video[0].nHeight > 1280 ||
                p->mMediaInfo.program[p->mMediaInfo.programIndex].video[0].nHeight == 0)
        ...
        if(InitializeVideoDecoder(p->mVideoDecoder,
            &p->mMediaInfo.program[p->mMediaInfo.programIndex].video[0], &vconfig) != 0)
        {
    ...
    do
    {
        ...
        ret = DecodeVideoStream(p->mVideoDecoder, 0 /*eos*/,
                1/*key frame only*/, 0/*drop b frame*/, 0/*current time*/);
