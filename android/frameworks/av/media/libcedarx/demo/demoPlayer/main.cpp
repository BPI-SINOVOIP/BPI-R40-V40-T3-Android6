...
static const Command commands[] =
{
    ...
    {"seek to",         COMMAND_SEEKTO,
            "seek to specific position to play, \
            position is in unit of second, for example, seek to: 100."},
    ...
    {"switch audio",    COMMAND_SWITCH_AUDIO,
        "switch audio to a specific track, for example, \
         switch audio: 2, track is start counting from 0."},
    {NULL, 0, NULL}
};
...
static void showHelp(void)
{
    ...
    printf("\n");
    printf("*****************************************************\n");
    printf("* This is a simple media player, when it is started, \
         you can input commands to tell\n");
...
static int parseCommandLine(char* strCommandLine, int* pParam)
{
    ...
    switch(nCommandId)
    {
        ...
        case COMMAND_SWITCH_AUDIO:
            ...
            //* audio stream index start counting from 0.
                if(errno == EINVAL || errno == ERANGE)
                {
    ...
    while(!bQuit)
    {
        ...
        if(readCommand(strCommandLine, sizeof(strCommandLine)) == 0)
        {
            ...
            switch(nCommandId)
            {
               ...
               //* if the media source is not seekable, this flag will be
               ...
               //* lock to sync with the prepare finish notify.
                ...
                case COMMAND_PLAY:   //* start playback.
                {
                  ...
                  //* current status is seeking, will set
