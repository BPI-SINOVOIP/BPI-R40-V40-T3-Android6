...
MediaInfo* DemuxCompGetMediaInfo(DemuxComp* d)
{
    ...
    {
        pVideoStreamInfo =
         (VideoStreamInfo*)malloc(sizeof(VideoStreamInfo)*myMediaInfo->nVideoStreamNum);
    ...
    {
        pAudioStreamInfo =
         (AudioStreamInfo*)malloc(sizeof(AudioStreamInfo)*myMediaInfo->nAudioStreamNum);
        ...
        for(i=0; i<myMediaInfo->nAudioStreamNum; i++)
        {
            ...
            if(pCodecSpecificData != NULL && nCodecSpecificDataLen > 0)
            {
                ...
                memcpy(pAudioStreamInfo->pCodecSpecificData,
                  pCodecSpecificData, nCodecSpecificDataLen);
    ...
    {
        pSubtitleStreamInfo =
       (SubtitleStreamInfo*)malloc(sizeof(SubtitleStreamInfo)*myMediaInfo->nSubtitleStreamNum);
...
process_message:
        ...
        else if(msg.messageId == DEMUX_COMMAND_PREPARE)
        {
            ...
            if(ret == 0)
            {
                ...
                if(demux->nSourceType == SOURCE_TYPE_URL)
                {
                  ...
                  //* also demux->mediaInfo.nFileSize == 0.
                        demux->bLiveStream = 1;
                    else
                        demux->bVodStream = 1;
                }
        ...
        else if(msg.messageId == DEMUX_COMMAND_START)
        {
            ...
            {
                loge("demux not in prepared or paused status \
                when DEMUX_COMMAND_START message received.");
                if(pReplyValue != NULL)
                    *pReplyValue = -1;
                if(pReplySem != NULL)
                    sem_post(pReplySem);
        ...
        else if(msg.messageId == DEMUX_COMMAND_SEEK)
        {
            ...
            if(demux->pParser != NULL)
            {
                ...
                if(ret == 0)
                {
                    ...
                    {
                        setMessage(&newMsg, DEMUX_COMMAND_READ);
                  //* send read message to start reading loop.
                        AwMessageQueuePostMessage(demux->mq, &newMsg);
                    }
               ...
               //* wait for 100ms if no message come.
                ...
                else
                {
                    ...
                    if(StreamCacheUnderflow(demux->pCache))
                    {
                        ...
                        //* wait for 50ms if no message come.
                                if(ret == 0)    //* new message come, quit loop to process.
                                    goto process_message;
                            }
                    ...
                    else
                    {
                     ...
                     //* wait for 200ms if no message come.
                            if(ret == 0)    //* new message come, quit loop to process.
                                goto process_message;
                        ...
                        else
                        {
                            ...
                            {
                                loge("media type from parser not valid, \
                             should not run here, abort().");
                                abort();
                            }
