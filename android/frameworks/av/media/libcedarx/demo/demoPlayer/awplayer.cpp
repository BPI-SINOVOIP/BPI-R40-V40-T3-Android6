       ...
       mStatus == AWPLAYER_STATUS_COMPLETE)
    {
        ...
        if(mMediaInfo != NULL)
        {
            ...
            || mMediaInfo->eContainerType == CONTAINER_TYPE_BD)
             ...
             //* generally, stream pts is started at 0 except ts stream.
...
int AwPlayer::getDuration(int *msec)
{
       ...
       mStatus == AWPLAYER_STATUS_COMPLETE)
    {
      ...
      //* in complete status, the prepare() method maybe called and
        if(mMediaInfo != NULL)
...
int AwPlayer::initializePlayer()
{
    ...
    if(mi->pAudioStreamInfo != NULL)
    {
        ...
        ret = PlayerSetAudioStreamInfo(mPlayer, mi->pAudioStreamInfo, mi->nAudioStreamNum,
                              nDefaultAudioIndex);
        if(ret != 0)
        {
...
int AwPlayer::callbackProcess(int messageId, void* param)
{
    switch(messageId)
    {
        ...
        case AWPLAYER_MESSAGE_DEMUX_CACHE_REPORT:
        {
            ...
            mNotifier(mUserData, NOTIFY_BUFFERRING_UPDATE, nBufferPercentage<<16
                  | nTotalPercentage, NULL);
            break;
        }
