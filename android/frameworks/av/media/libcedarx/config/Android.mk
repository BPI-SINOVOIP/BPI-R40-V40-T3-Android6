...
ifeq ($(CONFIG_OS), $(OPTION_OS_ANDROID))
    ...
    else ifeq (cheetah, $(product)) #H5
        ifeq ($(CONFIG_CMCC), $(OPTION_CMCC_YES))
            LOCAL_SRC_FILES := cheetah_cmcc_cedarx.conf
        else
            LOCAL_SRC_FILES := cheetah_cedarx.conf
        endif
	...
	else ifeq (t3, $(product)) #t3 androidm
        LOCAL_SRC_FILES := t3_cedarx.conf
    else
