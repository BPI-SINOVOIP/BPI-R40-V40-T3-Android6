...
//    AwPluginInit();
    ...
    mPriData->mParser             = NULL;
    mPriData->mStream                = NULL;
    mPriData->mVideoDecoder       = NULL;
    ...
    mPriData->memops = MemAdapterGetOpsS();
    CdcMemOpen(mPriData->memops);
}
...
void AwMetadataRetriever::clear()
{
    ...
    if(mPriData->mParser)
    {
        CdxParserForceStop(mPriData->mParser);
        CdxParserClose(mPriData->mParser);
        mPriData->mParser = NULL;
        mPriData->mStream = NULL;
    }
    else if(mPriData->mStream)
    {
        CdxStreamForceStop(mPriData->mStream);
        CdxStreamClose(mPriData->mStream);
        mPriData->mStream = NULL;
    }

    if(mPriData->mVideoDecoder != NULL)
    {
        DestroyVideoDecoder(mPriData->mVideoDecoder);
        mPriData->mVideoDecoder = NULL;
    }
    ...
    clearDataSourceFields(&mPriData->mSource);
...
#endif
{
    ...
    if( setDataSourceFields(&mPriData->mSource, (char*)pUrl,
                (KeyedVector<String8, String8>*)pHeaders) != 0)
    {
        loge("initialize media source for parser fail.");
        return NO_MEMORY;
    }
    //* 2. create a parser.
    mPriData->mStream = CdxStreamCreate(&mPriData->mSource);
    if(!mPriData->mStream)
    {
        loge("stream creat fail.");
        return UNKNOWN_ERROR;
    }
    int ret = CdxStreamConnect(mPriData->mStream);
    if(ret < 0)
    {
        return UNKNOWN_ERROR;
    }
    mPriData->mParser = CdxParserCreate(mPriData->mStream, MUTIL_AUDIO);
    if(!mPriData->mParser)
    {
        loge("parser creat fail.");
        return UNKNOWN_ERROR;
    }
    ret = CdxParserInit(mPriData->mParser);
    if(ret < 0)
    {
        return UNKNOWN_ERROR;
    }
    ...
    //* 3. get media info.
    memset(&mPriData->mMediaInfo, 0, sizeof(CdxMediaInfoT));
    if(CdxParserGetMediaInfo(mPriData->mParser, &mPriData->mMediaInfo) == 0)
    {
        if (mPriData->mParser->type == CDX_PARSER_TS ||
            mPriData->mParser->type == CDX_PARSER_BD ||
            mPriData->mParser->type == CDX_PARSER_HLS)
        {
            mPriData->mMediaInfo.program[0].video[0].bIsFramePackage = 0; /* stream package */
        }
        else
        {
            mPriData->mMediaInfo.program[0].video[0].bIsFramePackage = 1; /* frame package */
        }
        storeMetadata();
    }
...
status_t AwMetadataRetriever::setDataSource(int fd, int64_t nOffset, int64_t nLength)
{
    ...
    mPriData->mSource.uri = strdup(str);
    if(mPriData->mSource.uri == NULL)
    {
        loge("initialize media source for parser fail.");
        return NO_MEMORY;
    }
...
sp<IMemory> AwMetadataRetriever::getStreamAtTime(int64_t timeUs)
{
    ...
    videoCodec =
        mPriData->mMediaInfo.program[mPriData->mMediaInfo.programIndex].video[0].eCodecFormat;
    ...
    if(mPriData->mVideoDecoder == NULL)
    {
        mPriData->mVideoDecoder = CreateVideoDecoder();
        ...
        //* set the rotation
        int nRotateDegree;
        int nRotation = atoi((const char*)mPriData->mMediaInfo.rotate);
        ...
        VideoStreamInfo *videoInfo =
                mPriData->mMediaInfo.program[mPriData->mMediaInfo.programIndex].video;
        ...
        if(InitializeVideoDecoder(mPriData->mVideoDecoder, videoInfo, &vconfig) != 0)
        {
    ...
    if(mPriData->mMediaInfo.bSeekable && timeUs != 0 && timeUs <
        ((int64_t)mPriData->mMediaInfo.program[mPriData->mMediaInfo.programIndex].duration*1000))
    {
        //* FIXME.
        ...
        if(CdxParserSeekTo(mPriData->mParser, timeUs) != 0)
        {
    ...
    do
    {
        while(VideoStreamFrameNum(mPriData->mVideoDecoder, 0) < 4)
        {
            //* 5.1 prefetch packet type and packet data size.
            if(CdxParserPrefetch(mPriData->mParser, &packet) != 0)
            {
            ...
            {
                ret = RequestVideoStreamBuffer(mPriData->mVideoDecoder,
                                               packet.length,
                                               (char**)&packet.buf,
                                               &packet.buflen,
                                               (char**)&packet.ringBuf,
                ...
                {
                    if(CdxParserRead(mPriData->mParser, &packet) == 0)
                    {
                        ...
                        SubmitVideoStreamData(mPriData->mVideoDecoder, &dataInfo, 0);
                    }
                    else
                    {
        ...
        logv("stream number: %d", VideoStreamFrameNum(mPriData->mVideoDecoder, 0));

        //* 5.3 decode stream.
        ret = DecodeVideoStream(mPriData->mVideoDecoder, 0 /*eos*/, 0/*key frame only*/,
        ...
        {
            pPicture = RequestPicture(mPriData->mVideoDecoder, 0/*the major stream*/);
            if(pPicture != NULL)
            {
                bFrameCounter++;
                int enc_width  = 0;
                ...
                if(bEncodeAllFrames || (bFrameCounter & 1))
                {
                    ...
                    sInputBuffer.pAddrPhyY =
                        (unsigned char*)CdcMemGetPhysicAddressCpu(mPriData->memops,
                                pPicture->pData0);
                ...
                ReturnPicture(mPriData->mVideoDecoder, pPicture);
            }
        }
...
EOS_EXIT:
    ...
    {
        sp<MemoryHeapBase> heap = new MemoryHeapBase(bBitStreamLength + 4, 0,
                                        "awmetadataretriever");
...
VideoFrame *AwMetadataRetriever::getFrameAtTime(int64_t timeUs, int option)
{
    ...
    if(mPriData->mVideoDecoder == NULL)
    {
        ...
        vconfig.nAlignStride        = 16;//* set align stride to 16 as defualt
#if 1
        /* set this flag when the parser can give this info, mov files recorded
         * by iphone or android phone conains this info.
         */
    ...
    int progIndex = mPriData->mMediaInfo.programIndex;
    if(timeUs < 0)
    {
        //The key frame of MKV always at the end of file, need reset to 0,
        //otherwise will return mix thumbnail
        if(mPriData->mParser->type == CDX_PARSER_MKV)
        {
            timeUs = 0;
        }
        else
        {
            timeUs = ((int64_t)mPriData->mMediaInfo.program[progIndex].duration*1000/2);
        }
    }

     if(mPriData->mMediaInfo.program[progIndex].duration < 30000)
     {
         timeUs = 0;
     }

    //* 3. seek parser to the specific position.
    if(mPriData->mMediaInfo.bSeekable && timeUs > 0 &&
            timeUs < ((int64_t)mPriData->mMediaInfo.program[progIndex].duration*1000))
    {
        //* FIXME.
        //* we should seek to a position according to the 'option' param.
        //* option = 0 means seek to the sync frame privious to the timeUs.
    ...
    do
    {
        ...
        {
            logi("video resolution changed.");
            ReopenVideoEngine(mPriData->mVideoDecoder, &vconfig,
                    &mPriData->mMediaInfo.program[progIndex].video[0]);
            continue;
        }
    ...
    if(bSuccess)
    {
        ...
        pVideoFrame->mSize = pVideoFrame->mWidth * pVideoFrame->mHeight * 2;
        ...
        if(transformPicture(pPicture, pVideoFrame, mPriData->memops) < 0)
        {
            delete pVideoFrame;
            return NULL;
        }
        ...
        struct ScMemOpsS *memops)
{
    ...
    pSrcY = (unsigned char*)pPicture->pData0 + pPicture->nTopOffset * pPicture->nLineStride +
        pPicture->nLeftOffset;

    if(pPicture->ePixelFormat== PIXEL_FORMAT_YV12)
    {
        pSrcV = (unsigned char*)pPicture->pData1 +
            (pPicture->nTopOffset/2) * (pPicture->nLineStride/2) + pPicture->nLeftOffset/2;
        pSrcU = (unsigned char*)pPicture->pData2 +
            (pPicture->nTopOffset/2) * (pPicture->nLineStride/2) + pPicture->nLeftOffset/2;
    }
    else
    {
