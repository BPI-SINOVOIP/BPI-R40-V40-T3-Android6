...
static void CreateFile(AwLogRecorder l)
{
    ...
    for(i=0; i<fileNum; i++)
    {
        ...
        //ALOGD("-- nFileStartTime = %lld, gettimeofday = %lld, timezone = %s",
        //      lc->nFileStartTime, tv.tv_sec * 1000000ll + tv.tv_usec, ptime.tm_zone);
 ...
 /*
     ...
     for(i=0; i<lc->nFileCount; i++)
     {
        ...
        {
            ALOGD("removeFileMin = %d, lc->strFilePath[nPos] = %s",
                removeFileMin, lc->strFilePath[nPos]);
            if(removeFileMin != -1)
            {
                remove(lastFileName);
...
static void* AwLogRecorderThread(void* arg)
{
    ...
    while(1)
    {
        ...
        //ALOGD("+++++ nTimeDiff = %lld, nCurTime = %lld, startTime = %lld",
        // nTimeDiff, nCurTime,lc->nFileStartTime);
        ...
        ALOGD("+++++ nTimeDiff = %lld, nCurTime = %lld, threadStartTime = %lld",
            nTimeDiff, nCurTime,lc->nThreadStartTime);
