...
static int MIN_PASSED_DATA_SIZE_KEPT  = (4*1024*1024);
...
{
    // In case CacheDataEnough = false and CacheOverflow = true
    if (nStartPlaySize > 0.5 * nMaxBufferSize)
        nStartPlaySize = 0.5 * nMaxBufferSize;

    assert(nMaxBufferSize > 0);
    assert(nStartPlaySize >= 0);

    pthread_mutex_lock(&c->mutex);
    c->nMaxBufferSize = nMaxBufferSize;
    c->nStartPlaySize = nStartPlaySize;
    // Todo: don't keep passed data for live stream
    MIN_PASSED_DATA_SIZE_KEPT = nMaxBufferSize * 0.3;
    pthread_mutex_unlock(&c->mutex);
    return;
}
...
int StreamCacheOverflow(StreamCache* c)
{
    ...
    bOverFlow = ((c->nDataSize + c->nPassedDataSize) >= c->nMaxBufferSize) &&
                (c->nPassedDataSize <= MIN_PASSED_DATA_SIZE_KEPT);
    pthread_mutex_unlock(&c->mutex);
    return bOverFlow;
}
