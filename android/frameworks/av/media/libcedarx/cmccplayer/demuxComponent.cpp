...
#else
static int setDataSourceFields(DemuxCompContext *demuxHdr,
        CdxDataSourceT* source, char* uri, KeyedVector<String8,String8>* pHeader);
#endif
...
DemuxComp* DemuxCompCreate(void)
{
    ...
    d->nStartPlayCacheTime     = GetConfigParamterInt("start_play_cache_time",6)*1000;
    ...
    switch(eCachePolicy)
    {
        ...
        default:
        {
            if (nStartPlaySize > 0)
            {
                ...
                logd("use specific nStartPlayCacheSize %d", nStartPlaySize);
            }
            if (nStartPlayTimeMs > 0)
            {
                demux->nStartPlayCacheTime = nStartPlayTimeMs;
                logd("use specific nStartPlayCacheTime %d", nStartPlayTimeMs);
            }
            if (nMaxBufferSize > 0)
            {
                /* Maybe we should add another parameter for live stream?
                 * There is no use case yet.
                 */
                demux->nCacheBufferSize = demux->nCacheBufferSizeLive = nMaxBufferSize;
                logd("use specific nCacheBufferSize %d", nMaxBufferSize);
            }
...
static int DemuxCompUseDefaultCacheParams(DemuxCompContext* demux)
{
    ...
    /* Stop calculating bitrate from filesize and duration, we already (and
     * should) done it in setMediaInfo()
     */
    if (demux->mediaInfo.nBitrate > 0)
        nBitrate = demux->mediaInfo.nBitrate;
    else if (demux->pPlayer != NULL)
        nBitrate = PlayerGetVideoBitrate(demux->pPlayer) +
            PlayerGetAudioBitrate(demux->pPlayer);

    if (nBitrate <= 0)
        nBitrate = 200 * 1024;
    else if (nBitrate <= 64*1024)
        nBitrate = 64*1024;
...
static void AdjsutCacheParams(DemuxCompContext* demux)
{
    ...
    if (demux->eCachePolicy == CACHE_POLICY_ADAPTIVE)
    {
        if(demux->mediaInfo.nFileSize <= 0)
        {
            if(demux->mediaInfo.nBitrate > 0)
                nBitrate = demux->mediaInfo.nBitrate;
            else  //* default vod stream bitrate is 200 kbits.
                nBitrate = PlayerGetVideoBitrate(demux->pPlayer) +
                    PlayerGetAudioBitrate(demux->pPlayer);
        }
        else
            ...
            */
            ...
            if(demux->bFileStream == 0)
            {
                ...
                DemuxCompSetCachePolicy((DemuxComp*)demux, CACHE_POLICY_QUICK,
                    demux->nStartPlayCacheSize, demux->nStartPlayCacheTime,
                    demux->nCacheBufferSize);
            }
            ...
            if(demux->bFileStream == 0)
            {
                DemuxCompSetCachePolicy((DemuxComp*)demux,
                            CACHE_POLICY_ADAPTIVE, 0, 0, 0);
            }
