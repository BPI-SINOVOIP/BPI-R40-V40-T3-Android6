...
status_t CameraClient::setPreviewWindow(const sp<IBinder>& binder,
        ...
        if (result != NO_ERROR) {
            ...
            if (mStopRend == true) {
                ...
                if (result == NO_ERROR) {
                    ...
                    ALOGE("try to set Window direct, old window = %p, new = %p ",
                          mPreviewWindow.get(), window.get());
    ...
    } else if (mStopRend == true) {
		ALOGD("mStopRend is true");
		/*background record*/
#endif
        if (window != 0) {
            native_window_set_scaling_mode(window.get(),
...
status_t CameraClient::setPreviewTarget(
    ...
    if (bufferProducer == NULL) {
        return setPreviewWindow(NULL, NULL);
    }
#endif
...
status_t CameraClient::startRecordingMode() {
	...
	//fengyun add for h264 start
	CameraParameters params = mHardware->getParameters();
	int caputreFomat =  params.getInt("capture-format");
	if(V4L2_PIX_FMT_H264 == caputreFomat)
	{
		// start recording mode
...
status_t CameraClient::setWaterMarkMultiple(const String8& mWaterMark, int dispMode){
	ALOGV("setWaterMarkMultiple mWaterMark = %s, dispMode = %d",mWaterMark.string(),dispMode);
	Mutex::Autolock lock(mLock);
	status_t result = checkPidAndHardware();
	if (result != NO_ERROR) return result;
	status_t ret = mHardware->setWaterMarkMultiple(mWaterMark,dispMode);
...
status_t CameraClient::sendCommand(int32_t cmd, int32_t arg1, int32_t arg2) {
    ...
    if (result != NO_ERROR) return result;
#ifdef START_STOP_RENDER
    if (cmd == CAMERA_CMD_START_RENDER) {
        mStopRend = false;
    } else if (cmd == CAMERA_CMD_STOP_RENDER) {
        mStopRend = true;
    } else if (cmd == CAMERA_CMD_SET_DISPLAY_ORIENTATION) {
#else
...
void CameraClient::setAnalogInputColor(int brightness,int contrast,int saturation)
{
	 mHardware->setAnalogInputColor(brightness,contrast,saturation);
}
// ----------------------------------------------------------------------------
