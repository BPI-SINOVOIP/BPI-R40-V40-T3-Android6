...
#ifdef ENABLE_MULTI_RECORD
    // added for multi record
    ALOGD("inputs size = %d", mInputs.size());

	audio_io_handle_t activeInput = mInputs.getActiveInput();
		if (activeInput != 0) {
			sp<AudioInputDescriptor> activeDesc = mInputs.valueFor(activeInput);
			if (activeDesc->mProfile  == profile) {
				ALOGI("input(%d) has the same profile", activeInput);
		 *input = activeInput;
                return NO_ERROR;
			}
		}
                                                       ...
                                                       bool fromCache)
{
    ...
    } else if (isStrategyActive(outputDesc, STRATEGY_EXTERNAL)) {
        device = getDeviceForStrategy(STRATEGY_EXTERNAL, fromCache);
...
audio_stream_type_t AudioPolicyManager::streamTypefromAttributesInt(const audio_attributes_t *attr)
{
    ...
    case AUDIO_USAGE_EXTERNAL:
	return AUDIO_STREAM_EXTERNAL;
