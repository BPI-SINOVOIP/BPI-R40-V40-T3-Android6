...
int  ScreenRecoveryUI::get_rotation()
{
...
void ScreenRecoveryUI::draw_background_locked(Icon icon) {
    ...
    if (icon) {
        ...
        textX = (gr_fb_width() - textWidth) / 2;
        textY = ((gr_fb_height() - (iconHeight+textHeight+40+sh)) / 2) + iconHeight + 40;
...
void ScreenRecoveryUI::draw_progress_locked() {
    ...
    if (currentIcon == INSTALLING_UPDATE || currentIcon == ERASING) {
        ...
        gr_blit(icon, 0, 0, gr_get_width(icon), gr_get_height(icon), iconX, iconY);
         if(rotation != 0) {
            GRSurface *text_surface = backgroundText[currentIcon];
            gr_color(255, 255, 255, 255);
            gr_texticon(textX, textY, text_surface);
         }
    }
