...
void *initialwaterMark(int wm_height)
{
    ...
    if (wm_ctrl == NULL) {
        ALOGE("<initialwaterMark> Alloc wm_ctrl error");
        return NULL;
    }
    ...
    for(i = 0; i < watermark_pic_num; ++i) {
        sprintf(filename, "/system/watermark/wm_%dp_%d.bmp", bmp_height, i);

        fp = fopen(filename, "r");
        if (NULL == fp) {
            ALOGE("<initialwaterMark>Fail to open file !", filename);
            goto OPEN_FILE_ERR;
        }
