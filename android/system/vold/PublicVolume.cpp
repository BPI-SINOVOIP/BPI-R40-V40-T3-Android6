...
*/
	...
	if(mDevPath.find("public:8") != std::string::npos)
	{
				//udisk_id = udisk_id/16 +1;
    ...
    if (fs_prepare_dir(mRawPath.c_str(), 0777, AID_ROOT, AID_ROOT) ||
	 ...
	 if(!vfat::Check(mDevPath))
	 {
   	 ...
   	 } /*else if(!iso9660::doMount(mDevPath.c_str(), mRawPath.c_str(), false, false, false,AID_MEDIA_RW, AID_MEDIA_RW, 0007,  false) ) {    
      		ALOGE("mount iso9660 sucess! %s\n", mDevPath.c_str());    
     } else if (!mount(mDevPath.c_str(), mRawPath.c_str(),"ext4", MS_NOATIME | MS_NODEV | MS_NODIRATIME, "")) {         
			ALOGE("%s mount via ext4/ext3/ext2 sucess! \n", mDevPath.c_str());
	 }*/ else {
