...
status_t ForceUnmount(const std::string& path) {
    ...
    const char* cpath = path.c_str();
	char buf[256];
	char *ploc=strstr(cpath,"runtime/write"); 
	memset(buf,0,sizeof(buf));
	if(ploc != 0)
	{		
		sprintf(buf,"/storage%s",ploc+strlen("runtime/write"));
		 LOG(WARNING) << "found: " << buf;
	}
	else
	{
		memcpy(buf,cpath,strlen(cpath));
	}

    if (!umount2(cpath, UMOUNT_NOFOLLOW|MNT_FORCE) || errno == EINVAL || errno == ENOENT) {
        return OK;
    }
    // Apps might still be handling eject request, so wait before
    // we start sending signals

    Process::killProcessesWithOpenFiles(buf, SIGINT);
    sleep(5);
    if (!umount2(cpath, UMOUNT_NOFOLLOW|MNT_FORCE) || errno == EINVAL || errno == ENOENT) {
        return OK;
    }

    Process::killProcessesWithOpenFiles(buf, SIGTERM);
    sleep(5);
    if (!umount2(cpath, UMOUNT_NOFOLLOW|MNT_FORCE) || errno == EINVAL || errno == ENOENT) {
        return OK;
    }
    ...
    Process::killProcessesWithOpenFiles(buf, SIGKILL);
    sleep(5);
    if (!umount2(cpath, UMOUNT_NOFOLLOW|MNT_FORCE) || errno == EINVAL || errno == ENOENT) {
        return OK;
    }
