...
int VolumeManager::linkPrimary(userid_t userId,char *name) {
    std::string source(mPrimary->getPath());
    if (mPrimary->getType() == android::vold::VolumeBase::Type::kEmulated) {
        source = StringPrintf("%s/%s", source.c_str(), name);
        fs_prepare_dir(source.c_str(), 0755, AID_ROOT, AID_ROOT);
    }

    std::string target(StringPrintf("/mnt/user/%s/primary", name));
    if (TEMP_FAILURE_RETRY(unlink(target.c_str()))) {
        if (errno != ENOENT) {
            SLOGW("Failed to unlink %s: %s", target.c_str(), strerror(errno));
        }
...
int VolumeManager::onUserStarted(userid_t userId,char *name) {
    // Note that sometimes the system will spin up processes from Zygote
    // before actually starting the user, so we're okay if Zygote
    // already created this directory.
    std::string path(StringPrintf("%s/%s", kUserMountPath, name));
    fs_prepare_dir(path.c_str(), 0755, AID_ROOT, AID_ROOT);

    mStartedUsers.insert(userId);
    if (mPrimary) {
        linkPrimary(userId,name);
    }
    return 0;
}
...
int VolumeManager::setPrimary(const std::shared_ptr<android::vold::VolumeBase>& vol,int forceuser) {
    mPrimary = vol;
    for (userid_t userId : mStartedUsers) {
		if(forceuser)
        	linkPrimary(userId,(char *)("sdcard"));
		else
			linkPrimary(userId);
    }
    return 0;
...
int VolumeManager::remountUid(uid_t uid, const std::string& mode) {
    ...
    struct stat sb;
    pid_t child;
	char force_Singleuser[PROPERTY_VALUE_MAX];
    ...
    while ((de = readdir(dir))) {
        ...
        if (!(child = fork())) {
			...
			property_get("ro.display.sdcard", force_Singleuser,"0");
		  
		       
		   
            // Mount user-specific symlink helper into place
			if (strcmp(force_Singleuser, "0") ) {
	            ...
	            std::string userSource(StringPrintf("/mnt/user/sdcard"));
	            if (TEMP_FAILURE_RETRY(mount(userSource.c_str(), "/storage/self",
	                    NULL, MS_BIND, NULL)) == -1) {
	                PLOG(ERROR) << "Failed to mount " << userSource << " for "
	                        << de->d_name;
