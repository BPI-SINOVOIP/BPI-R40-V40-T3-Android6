...
build_gpu()
{
	...
	if [ "X${LICHEE_PLATFORM}" = "Xandroid" -o "X${LICHEE_PLATFORM}" = "Xsecureandroid" -o "X${LICHEE_PLATFORM}" = "Xandroidm"  ] ; then
	{
		TMP_OUT=$OUT
		TMP_TOP=$TOP
		unset OUT
...
gen_output()
{
	if [ "x${LICHEE_PLATFORM}" = "xandroid" ] || [ "x${LICHEE_PLATFORM}" = "xandroidm" ] ; then
