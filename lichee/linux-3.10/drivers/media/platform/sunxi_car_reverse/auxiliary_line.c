...
typedef void (*colormap_func)(int x, int y, int rotate, struct argb_t *out);
...
static int draw_by_func(struct canvas *ca, int rotate, colormap_func func)
...
static void colormap(int x, int y, int rotate, struct argb_t *out)
...
int draw_auxiliary_line(void *base, int width, int height, int rotate)

{
	...
	draw_by_func(&canvas, rotate, colormap);
}
