...
s32 tvd_get_signal (u32 sel)
{
	__u32 reg_val = 0;

	if(tvd_device[sel]->tvd_status4.bits.no_signal_detected == 1)
		reg_val= 1;
	else
		reg_val = 0;
...
s32 tvd_config(u32 sel,u32 interface,u32 system)
{
	...
	//tvd_device[sel]->tvd_hlock5.bits.hactive_start = (system?133:130) - (interface?20:0);
	tvd_device[sel]->tvd_hlock5.bits.hactive_start = (system ? 137 : 137) -
							(interface ? 20 : 0);
