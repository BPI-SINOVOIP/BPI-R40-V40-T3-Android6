			      ...
			      struct cfg80211_ap_settings *params)
{
	...
	sdata = IEEE80211_DEV_TO_SUB_IF(dev);
	/* not support ht40,also is NL80211_CHAN_HT40MINUS and NL80211_CHAN_HT40PLUS */
	/*in wireless,NL80211_CHAN_HT40MINUS,NL80211_CHAN_HT40PLUS replaced by chandef.width and chandef.center_freq1*/
	if(params->chandef.width > NL80211_CHAN_WIDTH_20)
		return -ENOTSUPP;
	ret = ieee80211_set_channel(wiphy,
					dev,params->chandef.chan, params->chandef.width);
	if(ret)
		return ret;
	old = rtnl_dereference(sdata->u.ap.beacon);
	if (old)
