...
static int sunxi_mmc_debdetect(struct mmc_host *host)
{
	u32 present = 0;
	int i = 0;
	int gpio_val = 0,det = 0;
	pr_debug("***%s %s %d***\n",mmc_hostname(host),__FUNCTION__,__LINE__);

       det = host->ops->get_cd(host);
       if(det)
       {
	       for (i=0; i<5; i++) {
...
*/
	...
	if((host->caps&MMC_CAP_NEEDS_POLL)){
		int cd_sta = 0;
		int cd_det = sunxi_mmc_debdetect(host);
		if(cd_det)
		{
			host->debounce++;
			for(i = 0 ;i <  5 ; i++)
			{
				msleep(200);
				cd_det = sunxi_mmc_debdetect(host);
				if(cd_det == 0)
				{
					host->debounce = 0;
					break;
...
#ifdef MMC_FORCE_REMOVE
		host->present_r = cd_sta;
		if (host->fremove)
		    cd_sta = 0;
#endif
		if((host->ops->get_cd)\
...
#ifdef MMC_FORCE_REMOVE
    if (host->fremove || (host->ops->get_cd && host->ops->get_cd(host) == 0)) {
#else
