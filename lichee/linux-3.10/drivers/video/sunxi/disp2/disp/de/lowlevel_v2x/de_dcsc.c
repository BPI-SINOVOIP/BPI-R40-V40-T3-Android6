...
extern void get_g_disp_drv_para(int sel,int *br,int *con,int *satu,int *hue);
...
int de_dcsc_apply(unsigned int sel, struct disp_csc_config *config)
{
	...
	get_g_disp_drv_para(sel,&csc_enhance_setting[0][0],&csc_enhance_setting[0][1],&csc_enhance_setting[0][2],&csc_enhance_setting[0][3]);
	config->enhance_mode =
    ...
    if(config->brightness == 0 && config->contrast  == 0 && config->saturation == 0 && config->hue == 0)
    {
	...
	__inf("lowleve_v2x:sel=%d, in_fmt=%d, mode=%d, out_fmt=%d, mode=%d, range=%d\n",
