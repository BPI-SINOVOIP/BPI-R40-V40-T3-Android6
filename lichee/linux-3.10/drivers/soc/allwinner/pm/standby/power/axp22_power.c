...
static struct axp_regulator_info axp22_regulator_info[] = {
	...
	AXP_LDO(AXP22, 12,   700, 1400, 100,  LDO12, 0, 3,  LDO12EN,
		0x01, 0x01, 0),
	AXP_LDO(AXP22, 16,  700, 3300, 100, LDOIO0, 0, 5, LDOIO0EN,
		0x07, 0x03, 0),
	AXP_LDO(AXP22, 17,  700, 3300, 100, LDOIO1, 0, 5, LDOIO1EN,
		0x07, 0x03, 0),
...
void axp22_losc_enter_ss(void)
{
	...
	printk("%s: dcdc_disable_val=0x%x, dcdc_enable_mask=0x%x\n",\
        ...
        //modify by golden
	ret = __twi_byte_update(AXP22_ADDR, AXP22_GPIO0_CTL,
			0x0,0x7);
	if (ret)
		printk("axp22 LDO gpio0 failed\n");

	ret = __twi_byte_update(AXP22_ADDR, AXP22_GPIO1_CTL,
			0x0,0x7);
	if (ret)
		printk("axp22 LDO gpio1 failed\n");
	//end modify
