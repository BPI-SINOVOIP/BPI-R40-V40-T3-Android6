...
static int sunxi_rtc_probe(struct platform_device *pdev)
{
	...
	int ret;
	unsigned int tmp_data = 0,w_data = 0,try_cnt = 0;
	...
	printk(KERN_ERR "rtc:before +++ tmp_data = 0x%x\n",tmp_data);

	tmp_data &= (~REG_CLK32K_AUTO_SWT_EN);
	tmp_data |= (EXT_LOSC_GSM);
	writel(tmp_data, chip->base + SUNXI_LOSC_CTRL);
	w_data |= (RTC_SOURCE_EXTERNAL | REG_LOSCCTRL_MAGIC);
	do
	{
		tmp_data = readl(chip->base + SUNXI_LOSC_CTRL);
		writel((w_data|tmp_data), chip->base + SUNXI_LOSC_CTRL);
		mdelay(1);
		tmp_data = readl(chip->base + SUNXI_LOSC_CTRL);
		try_cnt ++;
		printk(KERN_ERR "rtc:try set bit 0 times=%d\n",try_cnt);
	}while((w_data & 0x1) != (tmp_data & 0x1) && (try_cnt < 10));
	device_init_wakeup(&pdev->dev, 1);
	tmp_data = readl(chip->base + SUNXI_LOSC_CTRL);
	printk(KERN_ERR "rtc:after +++ tmp_data = 0x%x\n",tmp_data);
