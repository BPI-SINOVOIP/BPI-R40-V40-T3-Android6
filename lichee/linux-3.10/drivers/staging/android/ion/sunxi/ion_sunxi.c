...
int sunxi_ion_probe(struct platform_device *pdev)
{
	...
	do{
		...
		if (ION_HEAP_TYPE_CARVEOUT == heaps_desc.type)  {
			...
			printk(KERN_ERR "reserved carvout base=0x%x,sz = 0x%x\n",base,size);
...
static int __init sunxi_ion_module_init(void)
{
	return platform_driver_register(&sunxi_ion_driver);
}

static void __exit sunxi_ion_module_exit(void)
{
	platform_driver_unregister(&sunxi_ion_driver);
}

subsys_initcall(sunxi_ion_module_init);
module_exit(sunxi_ion_module_exit);
