...
const char *get_pmu_cur_name(void)
{
...
int axp_mfd_cell_name_init(struct axp_platform_ops *ops,
				int size, struct mfd_cell *cells)
{
	int i, j, find = 0;

	for (j = 0; j < ARRAY_SIZE(ops->regulator_name); j++) {
		if (strstr(ops->regulator_name[j], axp_name) != NULL) {
			find = 1;
			break;
		}
	}
			...
			struct axp_config_info *axp_config)
{
	...
	if (of_property_read_string(node, "compatible", &axp_name)) {
