	...
	do
	{
		...
		else if(ret == 0)
		{
			...
			if(monitor_entry)
			{
				...
				key_status = sunxi_probe_uart_input();
				if (key_status == '2')
					return -1;

				bfh->boot_ext[0].data[1] = key_status;
...
//			ndump(sub_certif.pubkey.e, sub_certif.pubkey.e_len);
		...
		if(item_group.binfile)
		{
			...
			} else if (!strcmp(item_group.binfile->name, ITEM_SCP_NAME)) {

				*(uint *)(SUNXI_BOOTLOGO_SOURCE_SIZE) = item_group.binfile->data_len;
				sunxi_flash_read(item_group.binfile->data_offset/512, (item_group.binfile->data_len+511)/512, (void *)SUNXI_BOOTLOGO_SOURCE_BUFFER);
			}
			else {//读出bin文件内容到内存
