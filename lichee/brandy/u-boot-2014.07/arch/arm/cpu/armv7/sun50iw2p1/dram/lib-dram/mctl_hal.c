...
//
//	History:
//				2016/04/6		WBK			0.10	Initial version
//*****************************************************************************
#include "mctl_reg.h"
#include "mctl_hal.h"
...
{
	unsigned int reg_val,i,j,ret_val ;
	unsigned int ic_dxN_rd_delay[4][11] = {{1,2,3,4,3,4,4,4,0,0,0},
	                                       {6,6,6,5,5,5,5,5,0,0,0},
	                                       {0,2,4,2,6,5,5,5,0,0,0},
	                                       {3,3,3,2,2,1,1,1,0,0,0}};

	unsigned int ic_dxN_wr_delay[4][11] = {{0,1,3,3,3,3,3,4,3,3,3},
	...
	for(i=0;i<4;i++)
	{
		for(j=0;j<11;j++)
		{
			ret_val =  ic_dxN_rd_delay[i][j]+(ic_dxN_wr_delay[i][j]<<8);
			reg_val = mctl_read_w((DATX0IOCR(j) + i * 0x80));
			reg_val += (ret_val<<0);
			mctl_write_w(reg_val,(DATX0IOCR(j) + i * 0x80));
		}
	}
...
void mctl_com_init(__dram_para_t *para)
{
	...
	for(m=0;m<rank_num;m++)
	{
		...
		reg_val &= ~(0xfff);
		reg_val |= ( (para->dram_para2)>>12 & 0x1);	//rank
...
unsigned int auto_scan_dram_size(__dram_para_t *para)
{
	...
	rank_num = (para->dram_para2>>8 & 0xf) + 1;
	for(m=0;m<rank_num;m++)
	{
		for(i=0;i<64;i++)
		{
...
unsigned int auto_scan_dram_type(__dram_para_t *para)
{
		...
		/*use minimum config to detect,row 11,bank 4,page 512B,full DQ */
		para->dram_tpr13 |= (0x1<<0);
		para->dram_para1 = 0x00B000B0; 
		para->dram_para2 = 0x0;
		if((para->dram_tpr13>>4)&0x1)
...
/*****************************************************************************
DRAMC保障机制功能块
1.dram_tpr13 bit29置1 关闭该机制，默认开启
2.将pad_hold标志置0
*****************************************************************************/
#ifdef USE_PROTECT_DRAM
	if(!((para->dram_tpr13>>29)&0x1)){
		ret_val = dramc_protect_function(para,pad_hold);
		pad_hold = 0;
		if(ret_val==0){
				dram_dbg_0("DRAM initial error : 3 !\n");
				return 0;
		}
	}
#endif
...
1.做驱动使用时，屏蔽该代码
2.LPDDR3/LPDDR2/DDR3参数在仿真及验证时自行修改
*****************************************************************************/
...
//	dram_para.dram_tpr12        = 0x0000cccc;
//	dram_para.dram_tpr13       	= 0x140062d1;
	/*DDR3参数*/
	...
	dram_para.dram_tpr8         = 0x0;
	dram_para.dram_tpr9         = 0x0;
	dram_para.dram_tpr10        = 0x2535;
	dram_para.dram_tpr11       	= 0x34440000;
	dram_para.dram_tpr12        = 0x00008988;
	dram_para.dram_tpr13       	= 0xC002191;
